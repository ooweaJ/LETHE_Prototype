...
function buildImplementationPrompt(context) {
  return [
    ...
    '3. 가능한 검증을 실행한다.',
    `4. \`docs/CODEX_STATUS.md\`, \`docs/NEXT_TASKS.md\`, \`docs/devlog/${date}.md\`, \`docs/reports/${date}.md\`를 갱신한다.`,
    `5. 보고서의 새 top-level heading은 반드시 \`# ${date}-NN - 작업 제목\` 형식으로 추가한다. 하루 전체 요약 제목을 새로 만들지 않는다.`,
    `6. 보고서 HTML과 단위별 보고 파일은 wrapper가 생성하므로 직접 커밋/푸시는 하지 않는다. \`npm run report\` 후 최신 단위가 \`docs/reports/units/${date}/\`에 Markdown/HTML로 분리되어야 한다.`,
    '',
    '## 현재 상태 발췌',
    '',
...
function runCodexTaskUpdate(iteration, feedbackPrompt) {
  ...
  const prompt = [
    ...
    `- 보고서의 새 top-level heading은 반드시 \`# ${date}-NN - 작업 제목\` 형식으로 추가한다.`,
