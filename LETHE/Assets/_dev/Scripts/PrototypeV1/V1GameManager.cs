...
namespace Lethe.PrototypeV1
{
    ...
    public sealed class V1GameManager : MonoBehaviour
    {
        ...
        const float KalmuriEchoVisibilityScale = 1.18f;
        ...
        const string EchoExecutionDualReworkPath = "Assets/_dev/Art/Sprites/Echoes/Execution/spr_execution_echo_dual_verdict_shred_01.png";
        const string EchoExecutionGreatReworkPath = "Assets/_dev/Art/Sprites/Echoes/Execution/spr_execution_echo_great_execution_gate_01.png";
        const string EchoShatterDualReworkPath = "Assets/_dev/Art/Sprites/Echoes/Shockwave/spr_shatter_echo_dual_fault_hop_01.png";
        const string EchoShatterGreatReworkPath = "Assets/_dev/Art/Sprites/Echoes/Shockwave/spr_shatter_echo_great_ground_collapse_01.png";
        const string EchoAshenDualReworkPath = "Assets/_dev/Art/Sprites/Echoes/Ashen/spr_ashen_echo_dual_parry_return_01.png";
        const string EchoAshenGreatReworkPath = "Assets/_dev/Art/Sprites/Echoes/Ashen/spr_ashen_echo_great_holy_wall_01.png";
        const string EchoOblivionDualReworkPath = "Assets/_dev/Art/Sprites/Echoes/Brand/spr_oblivion_echo_dual_erasure_scars_01.png";
        const string EchoOblivionGreatReworkPath = "Assets/_dev/Art/Sprites/Echoes/Brand/spr_oblivion_echo_great_void_collapse_01.png";
        ...
        readonly List<ActiveStoppedField> activeStoppedFields = new();
        ...
        void CreateAudio()
        {
            ...
            sfxClips["shatter"] = MakeSfxClip("sfx_shatter", 96f, 52f, 0.28f, 0.34f, V1SfxWave.Square, 0.26f, 0.004f, 1.15f);
        ...
        void RegisterStoppedField(Vector3 center, float radius, float duration, float freezeSeconds, bool strong)
        {
            activeStoppedFields.Add(new ActiveStoppedField(center, radius, Mathf.Max(0.10f, duration), Mathf.Max(0.10f, freezeSeconds), strong));
        }

        void UpdateActiveStoppedFields(float dt)
        {
            for (int i = activeStoppedFields.Count - 1; i >= 0; i--)
            {
                var field = activeStoppedFields[i];
                field.Remaining -= dt;
                if (field.Remaining <= 0f)
                {
                    activeStoppedFields.RemoveAt(i);
                    continue;
                }

                var radius = field.Radius;
                var pulseFreeze = Mathf.Min(field.FreezeSeconds, field.Strong ? 0.18f : 0.14f);
                QueryLivingEnemiesInRadius(field.Center, radius + 0.65f, radiusQueryBuffer);
                for (int j = 0; j < radiusQueryBuffer.Count; j++)
                {
                    var enemy = radiusQueryBuffer[j];
                    if (enemy == null || !enemy.IsAlive) continue;
                    if (Vector2.Distance(field.Center, enemy.transform.position) > radius + enemy.TouchRadius) continue;
                    enemy.ApplyBriefFreeze(pulseFreeze);
                }

                activeStoppedFields[i] = field;
            }
        }
        ...
        void UpdateExecutionFlash(MemoryState memory, float dt)
        {
            ...
            SpawnExecutionCondemnation(target.transform.position, (Vector2)(target.transform.position - player.position), memory.Level, false, true);
        ...
        {
            PlaySfx("shatter", echo ? 0.42f : 0.70f, echo ? 0.16f : 0.22f);
            ...
            var crackCount = echo ? 7 : 9;
            for (int i = 0; i < crackCount; i++)
            {
                var lane = i - (crackCount - 1) * 0.5f;
                var angle = -90f + lane * (echo ? 18f : 16f) + Mathf.Sin(i * 1.9f + elapsed) * 7f;
                var dir = Quaternion.Euler(0f, 0f, angle + 90f) * Vector3.right;
                var pos = center + dir * radius * (0.12f + Mathf.Abs(lane) * 0.055f);
                var len = radius * Mathf.Lerp(0.34f, echo ? 0.72f : 0.84f, 1f - Mathf.Abs(lane) / Mathf.Max(1f, crackCount * 0.5f));
                SpawnTransientSpriteScaled($"{prefix}_GroundCrack", crack, pos, Quaternion.Euler(0f, 0f, angle), new Vector3(echo ? 0.026f : 0.034f, len, 1f), new Color(0.68f, 0.98f, 1f, echo ? 0.52f : 0.64f), lifetime * (0.62f + i * 0.018f));
                if (i % 2 == 0)
                {
                    SpawnTransientSprite($"{prefix}_LiftedShard", shard, pos + dir * radius * 0.08f, Quaternion.Euler(0f, 0f, angle + 45f), echo ? 0.12f : 0.16f, new Color(0.82f, 1f, 1f, echo ? 0.48f : 0.62f), lifetime * 0.42f);
                }
            }
        }
        ...
        void SpawnStoppedEchoClamp(Vector3 center, float radius, float lifetime)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                SpawnHeldTransientSpriteScaled("StoppedEchoClampTick", tick, center + dir * radius * 0.72f, Quaternion.Euler(0f, 0f, angle), new Vector3(0.024f, 0.22f, 1f), new Color(color.r, color.g, color.b, 0.62f), lifetime, 0.82f);
            }
        }
        ...
        void SpawnOblivionBrandGlyph(Vector3 center, Vector2 forward, int levelValue, bool echo, bool rupture, float lifetime)
        {
            ...
            var prefix = echo ? "EchoOblivionBrandGlyph" : "MemoryOblivionBrandGlyph";
            ...
            SpawnTransientSpriteScaled($"{prefix}_SentenceCut", slash, center + (Vector3)(f * 0.12f), Quaternion.Euler(0f, 0f, baseAngle - 90f), new Vector3(0.022f * scale, 0.96f * scale, 1f), new Color(1f, 0.72f, 1f, violet.a * 0.70f), lifetime * 0.70f);

            var shardCount = rupture ? 6 : 4;
            for (int i = 0; i < shardCount; i++)
            {
                var lane = i - (shardCount - 1) * 0.5f;
                var pos = center + (Vector3)(s * lane * 0.085f * scale + f * (0.10f + Mathf.Abs(lane) * 0.035f));
                SpawnTransientSprite($"{prefix}_BrokenShard", shard, pos, Quaternion.Euler(0f, 0f, 45f + baseAngle + i * 31f), 0.12f * scale, new Color(0.92f, 0.56f, 1f, rupture ? 0.70f : 0.54f), lifetime * (0.54f + i * 0.018f));
            }
        }
        ...
        void UpdateStoppedSecond(MemoryState memory, float dt)
        {
            ...
            RegisterStoppedField(center, radius, 1.75f, freezeDuration, true);
        ...
        void UpdateAshenShield(MemoryState memory, float dt)
        {
            ...
            if (memory.Level >= 5)
            {
                SpawnAshenHolyFlames(player.position, lastAim, memory.Level, true, true);
        ...
        void UpdateOblivionBrand(MemoryState memory, float dt)
        {
            ...
            foreach (var enemy in enemies.Where(e => e != null && e.IsAlive).OrderBy(_ => UnityEngine.Random.value).Take(brandCount).ToList())
            {
                ...
                StartCoroutine(ResolveOblivionDelayedErase(enemy, player.position, memory.Level, memory.Level >= 5, 0.30f, "OblivionBrand delayed erase"));
            }
        }

        IEnumerator ResolveOblivionDelayedErase(V1Enemy target, Vector3 source, int levelValue, bool rupture, float delay, string sourceName)
        {
            if (delay > 0f) yield return new WaitForSeconds(delay);
            if (target == null || !target.IsAlive) yield break;

            var center = target.transform.position;
            var dir = (Vector2)(center - source);
            var f = dir.sqrMagnitude > 0.01f ? dir.normalized : lastAim;
            PlaySfx("brand", rupture ? 0.54f : 0.42f, 0.06f);
            SpawnOblivionEraseBurst(center, f, 0.62f + levelValue * 0.065f, rupture, sourceName);
            SpawnEchoHitRead(V1MemoryId.OblivionBrand, source, target, f, rupture, levelValue);
            MarkEnemyEchoState(target, V1MemoryId.OblivionBrand, rupture ? 1.35f : 1.05f, rupture ? 1.10f : 0.94f);
            DealDamage(target, 6.0f + levelValue * (rupture ? 3.2f : 2.2f), sourceName, false, f, rupture ? 0.28f : 0.12f);

            if (levelValue < 3) yield break;

            var spreadRadius = rupture ? 1.74f + levelValue * 0.12f : 1.22f + levelValue * 0.08f;
            var spreadCap = rupture ? 4 : 2;
            var linkedTargets = enemies
                .Where(e => e != null && e.IsAlive && e != target && Vector2.Distance(center, e.transform.position) <= spreadRadius + e.TouchRadius)
                .OrderBy(e => Vector2.Distance(center, e.transform.position))
                .Take(spreadCap)
                .ToList();
            for (int i = 0; i < linkedTargets.Count; i++)
            {
        ...
        IEnumerator ResolveOblivionEchoErase(V1Enemy target, Vector3 source, float damage, int levelValue, bool heavy, float delay, string sourceName)
        {
            if (delay > 0f) yield return new WaitForSeconds(delay);
            if (target == null || !target.IsAlive) yield break;
            ...
            var spreadTargets = enemies
                .Where(e => e != null && e.IsAlive && e != target && Vector2.Distance(center, e.transform.position) <= (heavy ? 1.58f : 1.22f) + levelValue * 0.08f + e.TouchRadius)
                .OrderBy(e => Vector2.Distance(center, e.transform.position))
                .Take(heavy ? 3 : 2)
                .ToList();
            foreach (var linked in spreadTargets)
            {
                var linkedDir = (Vector2)(linked.transform.position - center);
        ...
        void SpawnExecutionForecast(V1Enemy enemy, int levelValue, bool echo)
        {
            ...
            SpawnExecutionCondemnation(pos, (Vector2)(pos - player.position), levelValue, echo, false);
        }

        void SpawnExecutionCondemnation(Vector3 center, Vector2 forward, int levelValue, bool echo, bool execute)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var prefix = execute ? (echo ? "ExecutionEchoCondemn" : "ExecutionMemoryCondemn") : (echo ? "ExecutionEchoForecast" : "ExecutionForecast");
            var lifetime = execute ? (echo ? 0.56f : 0.50f) : (echo ? 0.42f : 0.34f);
            var scale = execute ? 1.00f + levelValue * 0.055f : 0.66f + levelValue * 0.035f;
            ...
            if (!echo)
            {
                SpawnMotifPromptSprite($"{prefix}_JudgementMotif", V1MemoryId.ExecutionFlash, false, () => MakeExecutionJudgementMotifSprite($"{prefix}_Motif", false, execute), center + (Vector3)(f * 0.05f), Quaternion.Euler(0f, 0f, baseAngle - 90f), motifScale * 1.92f, motifScale, new Color(1f, 1f, 1f, execute ? 0.98f : 0.72f), lifetime);
            }
            ...
            SpawnTransientSpriteScaled($"{prefix}_SentenceBarB", bar, center - (Vector3)(s * 0.18f * scale), Quaternion.Euler(0f, 0f, baseAngle - 176f), new Vector3(0.032f * scale, 0.54f * scale, 1f), new Color(gold.r, gold.g, gold.b, gold.a * 0.74f), lifetime * 0.82f);
            SpawnTransientSprite($"{prefix}_NailCore", mark, center + Vector3.up * 0.05f, Quaternion.Euler(0f, 0f, 45f + elapsed * (execute ? 220f : 90f)), 0.22f * scale, gold, lifetime * 0.72f);
            if (execute)
            {
                SpawnRadialSlashLines($"{prefix}_SentenceBurst", center, f, 6, 0.76f + levelValue * 0.055f, white, lifetime * 0.74f);
            }
        }
        ...
        void AddAshenGuardCharge(float amount, int levelValue, Vector3 origin, bool quiet)
        {
            ...
            if (!quiet)
            {
                SpawnAshenHolyFlames(origin, lastAim, levelValue, false, false);
            }
        }
        ...
        void ReleaseAshenGuardWave(Vector3 center, int levelValue, float charge, bool heavy, string source)
        {
            ...
            SpawnAshenHolyFlames(center, lastAim, levelValue, true, heavy);
            SpawnRadialSlashLines(heavy ? "AshenStoredWaveShardHeavy" : "AshenStoredWaveShard", center, lastAim, heavy ? 9 : 6, radius * 0.62f, new Color(1f, 0.94f, 0.68f, heavy ? 0.70f : 0.52f), heavy ? 0.42f : 0.30f);
            var counterTargets = AshenGuardCounterTargets(center, radius, heavy, levelValue);
            for (int i = 0; i < counterTargets.Count; i++)
            {
                var target = counterTargets[i];
                var dir = (Vector2)(target.transform.position - center);
                ...
                DealDamage(target, damage * (i == 0 ? 1f : heavy ? 0.56f : 0.42f), source, false, dir.sqrMagnitude > 0.01f ? dir.normalized : Vector2.up, heavy ? 1.15f : 0.62f);
            }
        ...
        {
            var cap = heavy ? Mathf.Clamp(2 + levelValue / 2, 4, 6) : Mathf.Clamp(1 + levelValue / 2, 2, 4);
            return enemies
                .Where(e => e != null && e.IsAlive && Vector2.Distance(center, e.transform.position) <= radius + e.TouchRadius)
                .OrderByDescending(EnemyThreatPriority)
                .ThenBy(e => Vector2.Distance(center, e.transform.position))
                .Take(cap)
                .ToList();
        }

        List<V1Enemy> AshenHolyWallTargets(Vector3 source, Vector2 forward, float range, float halfWidth, int cap)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            return enemies
                .Where(e =>
                {
                    ...
                    var delta = (Vector2)(e.transform.position - source);
                    var along = Vector2.Dot(delta, f);
                    var across = Mathf.Abs(Vector2.Dot(delta, s));
                    return along >= -0.18f && along <= range + e.TouchRadius && across <= halfWidth + e.TouchRadius;
                })
                .OrderBy(e => Vector2.Dot((Vector2)(e.transform.position - source), f))
                .ThenByDescending(EnemyThreatPriority)
                .Take(cap)
                .ToList();
        }
        ...
        List<V1Enemy> EchoTargetsInRadius(Vector3 origin, float radius, int cap, V1Enemy first = null)
        {
            var result = new List<V1Enemy>();
        ...
        List<V1Enemy> EchoTargetsInCone(Vector3 origin, Vector2 forward, float range, float halfAngle, int cap, V1Enemy first = null)
        {
            var f = EchoForward(forward);
            var result = new List<V1Enemy>();
            if (first != null && first.IsAlive) result.Add(first);
        ...
        List<V1Enemy> EchoChainTargets(V1Enemy first, int cap, float hopRadius)
        {
            var result = new List<V1Enemy>();
            if (first == null || !first.IsAlive) return result;
            result.Add(first);
            var cursor = first;
        ...
        void TriggerBloodEchoAccent(V1Enemy enemy, Vector2 forward, int levelValue, int hitIndex, WeaponRuntimeSpec weapon, bool force)
        {
            ...
            {
                var side = new Vector2(-f.y, f.x);
                var chain = EchoChainTargets(enemy, Mathf.Min(5, 2 + levelValue / 2), 1.15f + levelValue * 0.08f);
                ...
                SpawnDualBloodEchoMutation(enemy.transform.position, f, side, levelValue, denseDualBlade);
                SpawnDualBloodStitchFan(enemy.transform.position, f, side, levelValue, denseDualBlade);
                for (int i = 0; i < Mathf.Min(4, 1 + levelValue / 2); i++)
                {
                    var offset = side * ((i - 1.5f) * 0.10f) + f * (0.05f * i);
                    SpawnEchoWoundSlash("EchoDual_BloodNeedleStack", enemy.transform.position + (Vector3)offset, f, new Color(1f, 0.16f, 0.22f, 0.56f), 0.56f + i * 0.09f, 0.24f);
                }
                for (int i = 0; i < chain.Count; i++)
                {
                    var target = chain[i];
                    var dir = i == 0 ? f : (Vector2)(target.transform.position - chain[i - 1].transform.position);
                    target.BloodMarked = true;
                    target.MarkTimer = Mathf.Max(target.MarkTimer, 1.55f + i * 0.16f);
                    if (i > 0 && !denseDualBlade)
                    {
                        SpawnEchoLink("EchoDual_BloodStitchHop", chain[i - 1].transform.position, target.transform.position, new Color(1f, 0.12f, 0.18f, 0.30f), 0.20f, 0.012f);
                    }
                    DealDamage(target, weapon.Damage * (0.026f + levelValue * 0.007f) * (i == 0 ? 1f : 0.72f), "Blood Echo Dual stitch", false, dir.sqrMagnitude > 0.01f ? dir.normalized : f, 0.04f);
                }
                HealPlayer(0.08f + chain.Count * (0.035f + levelValue * 0.006f));
            }
        ...
        void TriggerGreatswordBloodIaido(V1Enemy enemy, Vector2 forward, int levelValue, WeaponRuntimeSpec weapon)
        {
            ...
            SpawnGreatswordBloodEchoMutation(slashCenter, slashForward, side, radius, levelValue);
            ...
            hitstopTimer = Mathf.Max(hitstopTimer, 0.055f);
            cameraShakeTimer = Mathf.Max(cameraShakeTimer, 0.18f);
            cameraShakeAmount = Mathf.Max(cameraShakeAmount, 0.088f);
        }
        ...
        void TriggerShatterEcho(V1Enemy enemy, Vector2 forward, int hitIndex, WeaponRuntimeSpec weapon, bool force)
        {
            ...
            if (heavy)
            {
                var origin = enemy.transform.position - (Vector3)(f * 0.18f);
                var impactCenter = enemy.transform.position + (Vector3)(f * 0.20f);
                var collapseRadius = radius * 1.16f;
                var fissureTargets = EchoTargetsInRadius(impactCenter, collapseRadius, EchoTargetLimit(V1MemoryId.ShatterWave, levelValue, heavy) + 3, enemy);
                SpawnGreatswordShatterGroundRupture(origin, f, radius, levelValue);
                SpawnGreatswordShatterEchoMutation(impactCenter, f, collapseRadius, levelValue);
                foreach (var target in fissureTargets)
                {
                    var dir = (Vector2)(target.transform.position - impactCenter);
                    var centerMul = Mathf.Lerp(1.22f, 0.76f, Mathf.Clamp01(dir.magnitude / Mathf.Max(0.01f, collapseRadius)));
                ...
                cameraShakeTimer = Mathf.Max(cameraShakeTimer, 0.18f);
                cameraShakeAmount = Mathf.Max(cameraShakeAmount, 0.082f);
                return;
            }

            var chain = denseDualBlade
                ? new List<V1Enemy> { enemy }
            ...
            if (!denseDualBlade)
            {
                ...
                SpawnDualShatterEchoMutation(chain, f, levelValue);
            }
            for (int i = 0; i < chain.Count; i++)
            {
                var target = chain[i];
        ...
        void TriggerExecutionEcho(V1Enemy enemy, Vector2 forward, int hitIndex, WeaponRuntimeSpec weapon, bool force)
        {
            ...
            if (heavy)
            {
                SpawnGreatswordExecutionEchoMutation(enemy.transform.position, f, levelValue);
                SpawnGreatswordGuillotineDrop(enemy.transform.position, f, levelValue);
                ...
                var verdictTargets = EchoTargetsInRadius(enemy.transform.position, 1.72f + levelValue * 0.16f, 7, enemy)
                    .Where(target => target == enemy || force || target.HealthRatio <= executeThreshold + 0.14f)
                    .OrderBy(target => target == enemy ? 0 : 1)
                    .ThenBy(target => target.HealthRatio)
                    .Take(force ? 4 : 3)
                    .ToList();
                for (int i = 0; i < verdictTargets.Count; i++)
                {
                    var target = verdictTargets[i];
                    var dir = (Vector2)(target.transform.position - enemy.transform.position);
                    var executionMul = i == 0 || target.HealthRatio < executeThreshold + 0.08f ? 1.22f : 0.46f;
                    ...
                    DealDamage(target, weapon.Damage * (target.HealthRatio <= executeThreshold + 0.08f ? 0.52f + levelValue * 0.084f : 0.12f + levelValue * 0.026f) * executionMul, i == 0 ? "Execution Echo Great verdict" : "Execution Echo Great witness cut", false, dir.sqrMagnitude > 0.01f ? dir.normalized : f, i == 0 ? 0.92f : 0.22f);
                }
            }
            else
            {
                var side = new Vector2(-f.y, f.x);
                var rawChain = denseDualBlade
                    ? new List<V1Enemy> { enemy }
                    : EchoChainTargets(enemy, Mathf.Min(5, 2 + levelValue / 2), 1.22f + levelValue * 0.08f);
                var chain = rawChain
                    .Where(target => target == enemy || force || target.HealthRatio <= executeThreshold + 0.16f)
                    .OrderBy(target => target == enemy ? 0 : 1)
                    .ThenBy(target => target.HealthRatio)
                    .Take(denseDualBlade ? 1 : Mathf.Min(4, 1 + levelValue / 2))
                    .ToList();
                if (!denseDualBlade)
                {
                    SpawnDualExecutionSentenceMarks(chain, f, levelValue);
                    SpawnDualExecutionEchoMutation(chain, f, levelValue);
                }
                SpawnDualBladeNeedleFan("EchoDual_ExecutionSentenceNeedle", enemy.transform.position, f, 0.72f + levelValue * 0.04f, 5, new Color(1f, 0.94f, 0.48f, 0.58f), 0.26f);
                for (int i = 0; i < 3; i++)
                {
        ...
        void TriggerHunterEcho(V1Enemy enemy, Vector2 forward, int hitIndex, WeaponRuntimeSpec weapon, bool force)
        {
            ...
            if (heavy)
            {
                MarkEnemyEchoState(enemy, V1MemoryId.HunterOath, 1.35f, 1.08f);
        ...
        void TriggerStoppedEcho(V1Enemy enemy, Vector2 forward, int hitIndex, WeaponRuntimeSpec weapon, bool force)
        {
            ...
            var freezeSeconds = Mathf.Max(1f, EchoFreezeSeconds(levelValue, heavy));
            var clockLifetime = freezeSeconds + 0.24f;
            RegisterStoppedField(enemy.transform.position, radius, clockLifetime, freezeSeconds, heavy);
            if (heavy)
            {
                SpawnHeldTransientSprite("EchoGreat_StoppedDome", MakeRingSprite("StoppedEchoWeaponTell", Color.white, 180), enemy.transform.position, Quaternion.identity, radius * 1.08f, new Color(1f, 0.76f, 0.22f, 0.58f), clockLifetime, 0.82f);
                SpawnGreatswordStoppedEchoMutation(enemy.transform.position, f, radius, levelValue, clockLifetime);
                SpawnStoppedEchoClamp(enemy.transform.position, 1.18f + levelValue * 0.10f, clockLifetime);
                SpawnStoppedSecondField(enemy.transform.position, radius, TimeStopGold(true), clockLifetime, true);
                SpawnStoppedEchoClockwork(enemy.transform.position, f, radius, levelValue, true, clockLifetime, freezeSeconds);
                SpawnGreatswordClockJudgement(enemy.transform.position, f, radius, levelValue);
            }
            else
            {
                var clockRadius = denseDualBlade ? radius * 0.72f : radius * 0.86f;
                SpawnStoppedSecondField(enemy.transform.position, clockRadius, TimeStopGold(false), clockLifetime, false);
                SpawnStoppedEchoClockwork(enemy.transform.position, f, clockRadius * 0.96f, levelValue, false, clockLifetime, freezeSeconds);
                if (!denseDualBlade) SpawnDualStoppedEchoMutation(enemy.transform.position, f, clockRadius, levelValue, clockLifetime);
            ...
            var stoppedTargets = heavy
                ...
                : EchoChainTargets(enemy, Mathf.Min(5, 2 + levelValue / 2), 1.18f + levelValue * 0.07f);
            foreach (var target in stoppedTargets)
            {
                var dir = (Vector2)(target.transform.position - enemy.transform.position);
                MarkEnemyEchoState(target, V1MemoryId.StoppedSecond, heavy ? 1.90f : 1.45f, heavy ? 1.08f : 0.94f);
                target.ApplyBriefFreeze(freezeSeconds);
        ...
        void TriggerAshenEcho(V1Enemy enemy, Vector2 forward, int hitIndex, WeaponRuntimeSpec weapon, bool force)
        {
            ...
            {
                SpawnAshenEchoCounterBurst(heavy ? player.position + (Vector3)(f * 0.42f) : enemy.transform.position, f, levelValue, heavy, denseDualBlade);
            }
            if (heavy || !denseDualBlade)
            {
            ...
            if (heavy)
            {
                ...
                SpawnGreatswordAshenBulwark(bulwarkCenter, f, levelValue);
                SpawnGreatswordAshenEchoMutation(bulwarkCenter, f, levelValue);
                SpawnTransientSpriteScaled("EchoGreat_AshenWallFace", MakeBoxSprite("EchoGreat_AshenWallFace", Color.white, 26, 150), bulwarkCenter + (Vector3)(f * 0.08f), Quaternion.Euler(0f, 0f, Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 90f), new Vector3(0.24f, 1.12f + levelValue * 0.05f, 1f), new Color(0.80f, 0.88f, 0.96f, 0.38f), 0.72f);
            }
            else if (!denseDualBlade)
            {
                SpawnDualAshenParrySparks(enemy.transform.position, f, levelValue);
                SpawnDualAshenEchoMutation(enemy.transform.position, f, levelValue);
                SpawnDualBladeNeedleFan("EchoDual_AshenParryNeedles", enemy.transform.position, f, 0.44f + levelValue * 0.025f, 4, new Color(0.88f, 0.96f, 1f, 0.44f), 0.22f);
            }
            if (heavy || !denseDualBlade)
            {
            ...
            if (heavy)
            {
                ...
                var wallRange = EchoRadius(V1MemoryId.AshenShield, levelValue, heavy) * 1.55f;
                var wallSource = player.position + (Vector3)(f * 0.68f);
                foreach (var target in AshenHolyWallTargets(wallSource, f, wallRange, 0.54f + levelValue * 0.055f, EchoTargetLimit(V1MemoryId.AshenShield, levelValue, heavy) + 1))
                {
                    var dir = (Vector2)(target.transform.position - wallSource);
                    ...
                    SpawnEchoHitRead(V1MemoryId.AshenShield, wallSource, target, dir.sqrMagnitude > 0.01f ? dir.normalized : f, true, levelValue);
                    DealDamage(target, weapon.Damage * EchoDamageMultiplier(V1MemoryId.AshenShield, levelValue, heavy) * distanceMul, "Ashen Echo Great holy wall lane", false, f, 0.58f);
                }
            }
            else
            {
                var parryTargets = denseDualBlade
                    ? new List<V1Enemy> { enemy }
                    : EchoChainTargets(enemy, Mathf.Min(4, 1 + levelValue / 2), 1.05f + levelValue * 0.08f);
                for (int i = 0; i < parryTargets.Count; i++)
                {
                    var target = parryTargets[i];
                    var dir = i == 0 ? f : (Vector2)(target.transform.position - parryTargets[i - 1].transform.position);
                    if (i > 0 && !denseDualBlade)
                    {
        ...
        void TriggerOblivionEcho(V1Enemy enemy, Vector2 forward, int hitIndex, WeaponRuntimeSpec weapon, bool force)
        {
            ...
            if (heavy)
            {
                var collapseRadius = EchoRadius(V1MemoryId.OblivionBrand, levelValue, true) * 1.18f;
                SpawnTransientSprite("EchoGreat_OblivionCoreBreak", MakeImpactDiamondSprite("EchoGreat_OblivionCoreBreak", Color.white), enemy.transform.position + Vector3.up * 0.08f, Quaternion.Euler(0f, 0f, elapsed * 70f), 0.74f, new Color(0.96f, 0.72f, 1f, 0.84f), 0.54f);
                SpawnRadialSlashLines("EchoGreat_OblivionBrandBurst", enemy.transform.position, f, 7, 1.26f + levelValue * 0.12f, new Color(0.86f, 0.48f, 1f, 0.70f), 0.52f);
                SpawnTransientSprite("EchoGreat_OblivionCollapseCoreShard", MakeImpactDiamondSprite("EchoGreat_OblivionCollapseCoreShard", Color.white), enemy.transform.position, Quaternion.Euler(0f, 0f, 45f + elapsed * -120f), collapseRadius * 0.46f, new Color(0.42f, 0.12f, 0.78f, 0.42f), 0.70f);
                SpawnGreatswordBrandCrater(enemy.transform.position, f, collapseRadius);
                SpawnGreatswordOblivionEchoMutation(enemy.transform.position, f, collapseRadius, levelValue);
                var brandTargets = EchoTargetsInRadius(enemy.transform.position, collapseRadius, EchoTargetLimit(V1MemoryId.OblivionBrand, levelValue, true), enemy)
                    .OrderBy(target => target == enemy ? 0 : 1)
                    .ThenBy(target => Vector2.Distance(enemy.transform.position, target.transform.position))
                    .Take(heavy ? 3 : 2)
                    .ToList();
                for (int i = 0; i < brandTargets.Count; i++)
                {
                    ...
                    DealDamage(target, weapon.Damage * (0.08f + levelValue * 0.018f), "Oblivion Echo Great inscription", false, dir.sqrMagnitude > 0.01f ? -dir.normalized : f, 0.10f);
                    StartCoroutine(ResolveOblivionEchoErase(target, enemy.transform.position, weapon.Damage * EchoDamageMultiplier(V1MemoryId.OblivionBrand, levelValue, true) * (i == 0 ? 1.10f : 0.62f), levelValue, true, 0.22f + i * 0.06f, i == 0 ? "Oblivion Echo Great collapse verdict" : "Oblivion Echo Great collapse witness"));
                }
            }
            else
            {
                if (!denseDualBlade)
                {
                    SpawnOblivionEraseBurst(enemy.transform.position, f, 0.54f + levelValue * 0.05f, false, "EchoDual_OblivionErase");
                    SpawnDualOblivionEchoMutation(enemy.transform.position, f, levelValue);
                }
                SpawnDualBrandHopRunes(enemy.transform.position, f, levelValue);
                SpawnDualBladeNeedleFan("EchoDual_OblivionStackCuts", enemy.transform.position, f, 0.52f + levelValue * 0.025f, 4, new Color(0.88f, 0.52f, 1f, 0.42f), 0.24f);
                for (int i = 0; i < Mathf.Min(4, 1 + levelValue / 2); i++)
                {
                    SpawnTransientSprite("EchoDual_OblivionStackPip", MakeImpactDiamondSprite("EchoDual_OblivionStackPip", Color.white), enemy.transform.position + Quaternion.Euler(0f, 0f, i * 90f + elapsed * 60f) * Vector3.right * 0.26f, Quaternion.Euler(0f, 0f, 45f + i * 30f), 0.20f, new Color(0.92f, 0.64f, 1f, 0.66f), 0.34f);
                }
                var stackTargets = EchoChainTargets(enemy, Mathf.Min(5, 2 + levelValue / 2), 1.22f + levelValue * 0.08f);
                for (int i = 0; i < stackTargets.Count; i++)
                {
                    var target = stackTargets[i];
                    var dir = i == 0 ? f : (Vector2)(target.transform.position - stackTargets[i - 1].transform.position);
                    if (i > 0)
                    {
                        SpawnEchoLink("EchoDual_OblivionStackHop", stackTargets[i - 1].transform.position, target.transform.position, new Color(0.82f, 0.46f, 1f, 0.34f), 0.22f, 0.012f);
                    }
                    if (!denseDualBlade)
                    {
                    ...
                    DealDamage(target, weapon.Damage * (0.035f + levelValue * 0.008f) * (i == 0 ? 1f : 0.70f), "Oblivion Echo Dual inscription", false, dir.sqrMagnitude > 0.01f ? dir.normalized : f, 0.04f);
                    StartCoroutine(ResolveOblivionEchoErase(target, i == 0 ? enemy.transform.position : stackTargets[i - 1].transform.position, weapon.Damage * EchoDamageMultiplier(V1MemoryId.OblivionBrand, levelValue, false) * (i == 0 ? 0.92f : 0.56f), levelValue, false, 0.16f + i * 0.045f, "Oblivion Echo Dual delayed scar"));
                }
                if (levelValue >= 5)
                {
        ...
        void SpawnHunterGreatEchoSpear(V1Enemy target, Vector3 origin, int levelValue, WeaponRuntimeSpec weapon)
        {
            ...
            go.transform.localScale = Vector3.one * (sr.sprite != null && sr.sprite.bounds.size.x > 2f ? ScaleSpriteToWorldWidth(sr.sprite, 1.58f) : 0.32f);
        ...
        void SpawnDualBladeNeedleFan(string name, Vector3 center, Vector2 forward, float length, int count, Color color, float lifetime)
        {
            var f = EchoForward(forward);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var needle = MakeBoxSprite(name, Color.white, 6, 116);
            count = Mathf.Clamp(count, 3, 7);
            for (int i = 0; i < count; i++)
            {
                var lane = count <= 1 ? 0f : (i - (count - 1) * 0.5f) / ((count - 1) * 0.5f);
                var angle = baseAngle + lane * 36f;
                var dir = (Vector2)(Quaternion.Euler(0f, 0f, angle) * Vector3.right);
                var pos = center + (Vector3)(dir * (0.16f + Mathf.Abs(lane) * 0.04f));
                SpawnTransientSpriteScaled(name, needle, pos, Quaternion.Euler(0f, 0f, angle - 90f), new Vector3(0.020f, Mathf.Max(0.22f, length), 1f), color, lifetime + i * 0.010f);
            }
        }

        void SpawnGreatswordHunterLanceTell(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var range = 1.42f + levelValue * 0.10f;
            SpawnTransientSpriteScaled("EchoGreat_HunterLanceBody", MakeBoxSprite("EchoGreat_HunterLanceBody", Color.white, 16, 190), center + (Vector3)(f * 0.42f), Quaternion.Euler(0f, 0f, baseAngle - 90f), new Vector3(0.090f, range, 1f), new Color(0.78f, 1f, 0.32f, 0.54f), 0.58f);
            SpawnTransientSprite("EchoGreat_HunterLanceHead", MakeImpactDiamondSprite("EchoGreat_HunterLanceHead", Color.white), center + (Vector3)(f * (0.98f + levelValue * 0.04f)), Quaternion.Euler(0f, 0f, baseAngle - 45f), 0.40f + levelValue * 0.018f, new Color(0.84f, 1f, 0.42f, 0.72f), 0.50f);
            SpawnGreatswordForwardWedge("EchoGreat_HunterLancePressure", center - (Vector3)(f * 0.18f), f, 1.55f + levelValue * 0.10f, 26f, new Color(0.62f, 1f, 0.26f, 0.16f), 0.54f);
        }

        void SpawnDualStoppedTicks(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var tick = MakeBoxSprite("EchoDual_StoppedNeedleTick", Color.white, 5, 72);
            var count = Mathf.Clamp(4 + levelValue / 2, 4, 7);
            for (int i = 0; i < count; i++)
            {
        ...
        void SpawnStoppedEchoClockwork(Vector3 center, Vector2 forward, float radius, int levelValue, bool heavy, float lifetime, float secondDuration)
        {
            var f = EchoForward(forward);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            ...
            SpawnClockHandStatic(heavy ? "EchoGreat_StoppedMinuteHandSettle" : "EchoDual_StoppedMinuteHandSettle", minuteHand, center, radius, baseAngle + 56f, heavy ? 0.054f : 0.046f, heavy ? 0.50f : 0.44f, new Color(gold.r, gold.g, gold.b, gold.a * 0.78f), lifetime);
            ...
            for (int i = 0; i < tickCount; i++)
            {
                ...
                SpawnHeldTransientSpriteScaled(
            ...
            SpawnHeldTransientSprite(heavy ? "EchoGreat_StoppedClockPin" : "EchoDual_StoppedClockPin", MakeImpactDiamondSprite("EchoStoppedClockPin", Color.white), center + (Vector3)(f * 0.03f), Quaternion.Euler(0f, 0f, baseAngle + 45f), heavy ? 0.22f : 0.18f, new Color(1f, 0.94f, 0.54f, heavy ? 0.86f : 0.76f), lifetime, 0.82f);
        }

        void SpawnDualBloodStitchFan(Vector3 center, Vector2 forward, Vector2 side, int levelValue, bool denseDualBlade)
        {
            if (denseDualBlade) return;
            var f = EchoForward(forward);
            var s = side.sqrMagnitude > 0.01f ? side.normalized : new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var needle = MakeBoxSprite("EchoDual_BloodStitchNeedle", Color.white, 6, 108);
            var count = Mathf.Clamp(3 + levelValue / 2, 4, 6);
            for (int i = 0; i < count; i++)
            {
                var lane = i - (count - 1) * 0.5f;
                var offset = s * (lane * 0.105f) + f * (0.05f + i * 0.018f);
                var angle = baseAngle - 90f + lane * 16f;
                SpawnTransientSpriteScaled(
                    "EchoDual_BloodStitchNeedle",
                    needle,
                    center + (Vector3)offset,
                    Quaternion.Euler(0f, 0f, angle),
                    new Vector3(0.024f, 0.44f + levelValue * 0.025f, 1f),
                    new Color(1f, 0.12f, 0.19f, 0.54f),
                    0.24f + i * 0.015f);
            }
        ...
        void SpawnDualBloodEchoMutation(Vector3 center, Vector2 forward, Vector2 side, int levelValue, bool dense)
        {
            if (dense) return;
            var f = EchoForward(forward);
            var s = side.sqrMagnitude > 0.01f ? side.normalized : new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var width = 0.48f + levelValue * 0.035f;
            SpawnPromptSprite("EchoDual_BloodSplitVortexLeft", LoadSprite(BloodVortexPath), () => MakeBloodVortexRingSprite("EchoDual_BloodSplitVortexLeftFallback", Color.white, true), center + (Vector3)(s * 0.16f + f * 0.05f), Quaternion.Euler(0f, 0f, baseAngle + 22f), width, 0.28f, new Color(1f, 0.92f, 0.94f, 0.42f), 0.34f);
            SpawnPromptSprite("EchoDual_BloodSplitVortexRight", LoadSprite(BloodVortexPath), () => MakeBloodVortexRingSprite("EchoDual_BloodSplitVortexRightFallback", Color.white, true), center - (Vector3)(s * 0.16f) + (Vector3)(f * 0.09f), Quaternion.Euler(0f, 0f, baseAngle - 36f), width * 0.92f, 0.25f, new Color(1f, 0.80f, 0.84f, 0.34f), 0.30f);
            for (int i = 0; i < 4; i++)
            {
                var lane = i - 1.5f;
                var pos = center + (Vector3)(s * lane * 0.11f + f * (0.10f + i * 0.035f));
                SpawnEchoWoundSlash("EchoDual_BloodSplitNeedleCut", pos, i % 2 == 0 ? f : -f, new Color(1f, 0.10f, 0.16f, 0.54f), 0.42f + levelValue * 0.030f, 0.22f + i * 0.010f);
            }
        }

        void SpawnGreatswordBloodEchoMutation(Vector3 center, Vector2 forward, Vector2 side, float radius, int levelValue)
        {
            var f = EchoForward(forward);
            var s = side.sqrMagnitude > 0.01f ? side.normalized : new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var slab = MakeBoxSprite("EchoGreat_BloodVortexRitualSlab", Color.white, 18, 190);
            SpawnTransientSpriteScaled("EchoGreat_BloodVortexRitualSpine", slab, center + (Vector3)(f * radius * 0.10f), Quaternion.Euler(0f, 0f, baseAngle - 90f), new Vector3(0.070f, radius * 1.18f, 1f), new Color(1f, 0.05f, 0.08f, 0.58f), 0.72f);
            SpawnTransientSpriteScaled("EchoGreat_BloodVortexRitualCrossA", slab, center + (Vector3)(s * radius * 0.05f), Quaternion.Euler(0f, 0f, baseAngle - 28f), new Vector3(0.046f, radius * 0.78f, 1f), new Color(1f, 0.92f, 0.88f, 0.42f), 0.58f);
            SpawnTransientSpriteScaled("EchoGreat_BloodVortexRitualCrossB", slab, center - (Vector3)(s * radius * 0.05f), Quaternion.Euler(0f, 0f, baseAngle + 34f), new Vector3(0.040f, radius * 0.64f, 1f), new Color(0.72f, 0.00f, 0.02f, 0.42f), 0.52f);
            SpawnRadialSlashLines("EchoGreat_BloodVortexRitualTeeth", center, f, 8 + Mathf.Min(4, levelValue), radius * 0.62f, new Color(1f, 0.12f, 0.12f, 0.60f), 0.54f);
        }
        ...
        void SpawnGreatswordBloodVortexRing(Vector3 center, Vector2 forward, Vector2 side, float radius, int levelValue)
        {
            ...
            SpawnTransientSprite("EchoGreat_BloodVortexPulse", MakeRingSprite("EchoGreat_BloodVortexPulse", Color.white, 180), center, Quaternion.identity, radius * 1.04f, new Color(1f, 0.04f, 0.08f, 0.54f), 0.44f);

            var shardCount = Mathf.Clamp(8 + levelValue, 9, 13);
            for (int i = 0; i < shardCount; i++)
            {
                var t = i / (float)shardCount;
                var angle = baseAngle + t * 360f + Mathf.Sin(i * 1.71f) * 13f;
                var dir = Quaternion.Euler(0f, 0f, angle) * Vector3.right;
                var tangentAngle = angle + 92f + (i % 2 == 0 ? 8f : -10f);
                var lane = i % 3 == 0 ? 1.05f : i % 3 == 1 ? 0.92f : 1.15f;
                var pos = center + dir * (radius * 0.72f * lane) + (Vector3)(s * Mathf.Sin(i * 0.93f) * 0.035f);
                var hot = i % 2 == 0;
                SpawnTransientSpriteScaled(
                    hot ? "EchoGreat_BloodVortexWhiteShard" : "EchoGreat_BloodVortexRedThread",
                    hot ? bladeShard : bloodShard,
                    pos,
                    Quaternion.Euler(0f, 0f, tangentAngle),
                    hot ? new Vector3(0.040f, 0.46f + levelValue * 0.014f, 1f) : new Vector3(0.030f, 0.38f + levelValue * 0.012f, 1f),
                    hot ? white : ember,
                    0.42f + i * 0.012f);
            }
        }

        void SpawnGreatswordForwardWedge(string name, Vector3 origin, Vector2 forward, float range, float angleDegrees, Color color, float lifetime)
        {
            var f = EchoForward(forward);
        ...
        void SpawnGreatswordShatterEchoMutation(Vector3 origin, Vector2 forward, float radius, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var impact = MakeBoxSprite("EchoGreat_ShatterAnvilDrop", Color.white, 24, 210);
            var center = origin;
            SpawnPromptSprite("EchoGreat_ShatterGroundCollapseSprite", LoadSprite(EchoShatterGreatReworkPath), () => MakeShatterSlamMotifSprite("EchoGreat_ShatterGroundCollapseFallback", true, true), center, Quaternion.Euler(0f, 0f, baseAngle - 90f), radius * 2.24f, radius * 0.88f, new Color(1f, 1f, 1f, 0.96f), 0.82f);
            SpawnTransientSpriteScaled("EchoGreat_ShatterAnvilDrop", impact, center + Vector3.up * 0.35f, Quaternion.Euler(0f, 0f, baseAngle - 90f), new Vector3(0.22f, radius * 1.34f, 1f), new Color(0.90f, 1f, 1f, 0.50f), 0.48f);
            SpawnEchoLink("EchoGreat_ShatterFaultLeft", origin + (Vector3)(s * 0.16f), center + (Vector3)(s * radius * 0.40f), new Color(0.56f, 0.96f, 1f, 0.42f), 0.58f, 0.026f);
            SpawnEchoLink("EchoGreat_ShatterFaultRight", origin - (Vector3)(s * 0.16f), center - (Vector3)(s * radius * 0.40f), new Color(0.56f, 0.96f, 1f, 0.36f), 0.58f, 0.022f);
            SpawnRadialSlashLines("EchoGreat_ShatterLiftedFangs", center, f, 6, radius * 0.58f, new Color(0.80f, 1f, 1f, 0.62f), 0.44f);
        }
        ...
        void SpawnDualShatterEchoMutation(List<V1Enemy> chain, Vector2 forward, int levelValue)
        {
            if (chain == null || chain.Count == 0) return;
            var f = EchoForward(forward);
            var chip = MakeImpactDiamondSprite("EchoDual_ShatterSkipChip", Color.white);
            var count = Mathf.Min(chain.Count, 5);
            for (int i = 0; i < count; i++)
            {
                var target = chain[i];
                if (target == null) continue;
                var dir = i > 0 && chain[i - 1] != null ? (Vector2)(target.transform.position - chain[i - 1].transform.position) : f;
                var angle = Mathf.Atan2(dir.y, dir.x) * Mathf.Rad2Deg;
                SpawnPromptSprite("EchoDual_ShatterFaultHopSprite", LoadSprite(EchoShatterDualReworkPath), () => MakeShatterSlamMotifSprite("EchoDual_ShatterFaultHopFallback", true, false), target.transform.position + Vector3.up * 0.02f, Quaternion.Euler(0f, 0f, angle - 20f), 0.84f + levelValue * 0.035f, 0.34f, new Color(1f, 1f, 1f, 0.78f), 0.30f + i * 0.014f);
                SpawnTransientSprite("EchoDual_ShatterSkipChip", chip, target.transform.position + Vector3.up * (0.06f + i * 0.01f), Quaternion.Euler(0f, 0f, angle + 45f + i * 24f), 0.12f + levelValue * 0.007f, new Color(0.78f, 1f, 1f, 0.58f), 0.26f + i * 0.014f);
                SpawnEchoWoundSlash("EchoDual_ShatterSkipScratch", target.transform.position, dir.sqrMagnitude > 0.01f ? dir.normalized : f, new Color(0.56f, 0.98f, 1f, 0.44f), 0.38f + i * 0.035f, 0.20f + i * 0.012f);
            }
        }
        ...
        void SpawnDualMicroRippleChain(Vector3 center, Vector2 forward, int levelValue)
        {
            ...
            {
                var lane = i - (count - 1) * 0.5f;
                var pos = center + (Vector3)(f * (0.08f + i * 0.055f) + s * (lane * 0.10f));
                SpawnTransientSprite("EchoDual_ShatterMicroRipple", MakeRingSprite("EchoDual_ShatterMicroRipple", Color.white, 96), pos, Quaternion.Euler(0f, 0f, elapsed * 140f + i * 35f), 0.22f + i * 0.022f, new Color(0.62f, 0.98f, 1f, 0.32f), 0.24f + i * 0.018f);
            }
        }

        void SpawnGreatswordGuillotineDrop(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
        ...
        void SpawnGreatswordExecutionEchoMutation(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var frame = MakeBoxSprite("EchoGreat_ExecutionGateFrame", Color.white, 16, 190);
            SpawnPromptSprite("EchoGreat_ExecutionGateSprite", LoadSprite(EchoExecutionGreatReworkPath), () => MakeExecutionJudgementMotifSprite("EchoGreat_ExecutionGateFallback", true, true), center + Vector3.up * 0.08f, Quaternion.Euler(0f, 0f, baseAngle - 90f), 2.26f + levelValue * 0.10f, 0.90f, new Color(1f, 1f, 1f, 0.96f), 0.70f);
            SpawnTransientSpriteScaled("EchoGreat_ExecutionGateLeft", frame, center + (Vector3)(s * 0.34f + Vector2.up * 0.12f), Quaternion.Euler(0f, 0f, baseAngle - 90f), new Vector3(0.052f, 1.20f + levelValue * 0.045f, 1f), new Color(0.34f, 0.20f, 0.04f, 0.52f), 0.62f);
            SpawnTransientSpriteScaled("EchoGreat_ExecutionGateRight", frame, center - (Vector3)(s * 0.34f) + Vector3.up * 0.12f, Quaternion.Euler(0f, 0f, baseAngle - 90f), new Vector3(0.052f, 1.20f + levelValue * 0.045f, 1f), new Color(0.34f, 0.20f, 0.04f, 0.46f), 0.62f);
            SpawnEchoLink("EchoGreat_ExecutionSentenceLintel", center + (Vector3)(s * 0.42f) + Vector3.up * 0.74f, center - (Vector3)(s * 0.42f) + Vector3.up * 0.74f, new Color(1f, 0.84f, 0.34f, 0.46f), 0.48f, 0.024f);
            SpawnRadialSlashLines("EchoGreat_ExecutionVerdictSplinters", center, f, 6, 0.74f + levelValue * 0.045f, new Color(1f, 0.90f, 0.42f, 0.56f), 0.42f);
        }

        void SpawnDualExecutionSentenceMarks(List<V1Enemy> chain, Vector2 forward, int levelValue)
        {
            if (chain == null || chain.Count == 0) return;
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
        ...
        void SpawnDualExecutionEchoMutation(List<V1Enemy> chain, Vector2 forward, int levelValue)
        {
            if (chain == null || chain.Count == 0) return;
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var bar = MakeBoxSprite("EchoDual_ExecutionVerdictBarcode", Color.white, 5, 78);
            var count = Mathf.Min(chain.Count, 5);
            for (int i = 0; i < count; i++)
            {
                var target = chain[i];
                if (target == null) continue;
                var center = target.transform.position + (Vector3)(f * 0.08f);
                SpawnPromptSprite("EchoDual_ExecutionVerdictShredSprite", LoadSprite(EchoExecutionDualReworkPath), () => MakeExecutionJudgementMotifSprite("EchoDual_ExecutionVerdictShredFallback", true, true), center, Quaternion.Euler(0f, 0f, Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 35f), 0.92f + levelValue * 0.030f, 0.38f, new Color(1f, 1f, 1f, 0.82f), 0.32f + i * 0.010f);
                for (int j = 0; j < 3; j++)
                {
                    var lane = j - 1f;
                    SpawnTransientSpriteScaled("EchoDual_ExecutionVerdictBarcode", bar, center + (Vector3)(s * (lane * 0.07f)), Quaternion.Euler(0f, 0f, Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 90f + lane * 7f), new Vector3(0.018f, 0.30f + levelValue * 0.012f, 1f), new Color(1f, 0.92f, 0.46f, 0.48f), 0.22f + i * 0.010f);
                }
            }
        }

        void SpawnDualHunterFan(Vector3 center, Vector2 forward, int levelValue, int targetCount)
        {
            var f = EchoForward(forward);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var count = Mathf.Clamp(Mathf.Max(targetCount, 3), 3, 5);
            for (int i = 0; i < count; i++)
            {
                var lane = count <= 1 ? 0f : (i - (count - 1) * 0.5f) / (count - 1);
                var angle = baseAngle + lane * 54f;
                var dir = (Vector2)(Quaternion.Euler(0f, 0f, angle) * Vector3.right);
                var start = center + (Vector3)(dir * 0.10f);
                var end = center + (Vector3)(dir * (0.72f + levelValue * 0.035f));
                SpawnEchoLink("EchoDual_HunterFanNeedleTrail", start, end, new Color(0.72f, 1f, 0.34f, 0.36f), 0.24f, 0.014f);
                SpawnTransientSpriteScaled("EchoDual_HunterFanNeedleHead", MakeBoxSprite("EchoDual_HunterFanNeedleHead", Color.white, 7, 96), end, Quaternion.Euler(0f, 0f, angle - 90f), new Vector3(0.028f, 0.44f, 1f), new Color(0.76f, 1f, 0.38f, 0.72f), 0.30f);
            }
        }

        void SpawnGreatswordClockJudgement(Vector3 center, Vector2 forward, float radius, int levelValue)
        {
            var f = EchoForward(forward);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var hand = MakeBoxSprite("EchoGreat_StoppedJudgementHand", Color.white, 10, 156);
            SpawnSweepingTransientSprite("EchoGreat_StoppedJudgementSweep", hand, center - (Vector3)(f * radius * 0.24f), center + (Vector3)(f * radius * 0.20f), baseAngle - 70f, baseAngle + 36f, 0.38f, 0.54f, new Color(1f, 0.82f, 0.34f, 0.74f), 0.76f, 0.38f);
            SpawnEchoWoundSlash("EchoGreat_StoppedLockedCut", center + (Vector3)(f * 0.10f), f, new Color(1f, 0.82f, 0.34f, 0.68f), 1.10f + levelValue * 0.06f, 0.42f);
        }

        void SpawnGreatswordStoppedEchoMutation(Vector3 center, Vector2 forward, float radius, int levelValue, float lifetime)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var bar = MakeBoxSprite("EchoGreat_StoppedCageBar", Color.white, 8, 180);
            SpawnHeldTransientSpriteScaled("EchoGreat_StoppedCageNorth", bar, center + Vector3.up * radius * 0.34f, Quaternion.Euler(0f, 0f, baseAngle - 90f), new Vector3(0.034f, radius * 0.76f, 1f), new Color(1f, 0.82f, 0.30f, 0.42f), lifetime, 0.82f);
            SpawnHeldTransientSpriteScaled("EchoGreat_StoppedCageSideA", bar, center + (Vector3)(s * radius * 0.48f), Quaternion.Euler(0f, 0f, baseAngle - 90f + 18f), new Vector3(0.028f, radius * 0.58f, 1f), new Color(1f, 0.82f, 0.30f, 0.34f), lifetime, 0.82f);
            SpawnHeldTransientSpriteScaled("EchoGreat_StoppedCageSideB", bar, center - (Vector3)(s * radius * 0.48f), Quaternion.Euler(0f, 0f, baseAngle - 90f - 18f), new Vector3(0.028f, radius * 0.58f, 1f), new Color(1f, 0.82f, 0.30f, 0.30f), lifetime, 0.82f);
            SpawnClockHandSweep("EchoGreat_StoppedCageSecondHand", MakeBoxSprite("EchoGreat_StoppedCageSecondHand", Color.white, 6, 170), center, radius, baseAngle - 120f, baseAngle + 240f, 0.040f, 0.88f, new Color(1f, 0.90f, 0.44f, 0.66f), lifetime, Mathf.Max(1f, lifetime - 0.24f));
        }

        void SpawnDualStoppedEchoMutation(Vector3 center, Vector2 forward, float radius, int levelValue, float lifetime)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var shard = MakeImpactDiamondSprite("EchoDual_StoppedBrokenClockShard", Color.white);
            var count = Mathf.Clamp(4 + levelValue / 2, 5, 7);
            for (int i = 0; i < count; i++)
            {
                var lane = i - (count - 1) * 0.5f;
                var pos = center + (Vector3)(s * lane * 0.105f + f * (0.05f + Mathf.Abs(lane) * 0.035f));
                SpawnHeldTransientSprite("EchoDual_StoppedBrokenClockShard", shard, pos, Quaternion.Euler(0f, 0f, 45f + i * 38f + elapsed * 80f), 0.11f + levelValue * 0.006f, new Color(1f, 0.88f, 0.36f, 0.48f), lifetime, 0.70f);
            }
            SpawnHeldTransientSpriteScaled("EchoDual_StoppedTwinSecondSlashA", MakeBoxSprite("EchoDual_StoppedTwinSecondSlash", Color.white, 5, 120), center + (Vector3)(s * 0.12f), Quaternion.Euler(0f, 0f, Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 36f), new Vector3(0.018f, radius * 0.62f, 1f), new Color(1f, 0.86f, 0.32f, 0.52f), lifetime, 0.70f);
            SpawnHeldTransientSpriteScaled("EchoDual_StoppedTwinSecondSlashB", MakeBoxSprite("EchoDual_StoppedTwinSecondSlash", Color.white, 5, 120), center - (Vector3)(s * 0.12f), Quaternion.Euler(0f, 0f, Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg + 36f), new Vector3(0.018f, radius * 0.52f, 1f), new Color(1f, 0.86f, 0.32f, 0.42f), lifetime, 0.70f);
        }

        void SpawnGreatswordAshenBulwark(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var angle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 90f;
        ...
        void SpawnGreatswordAshenEchoMutation(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var angle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 90f;
            var pillar = MakeBoxSprite("EchoGreat_AshenCathedralPillar", Color.white, 16, 150);
            SpawnPromptSprite("EchoGreat_AshenHolyWallSprite", LoadSprite(EchoAshenGreatReworkPath), () => MakeAshenHolyFireMotifSprite("EchoGreat_AshenHolyWallFallback", true, true), center + (Vector3)(f * 0.10f), Quaternion.Euler(0f, 0f, angle), 2.36f + levelValue * 0.08f, 0.94f, new Color(1f, 1f, 1f, 0.92f), 0.72f);
            for (int i = -1; i <= 1; i++)
            {
                var pos = center + (Vector3)(s * i * (0.26f + levelValue * 0.012f) + f * 0.04f);
                SpawnTransientSpriteScaled("EchoGreat_AshenCathedralPillar", pillar, pos, Quaternion.Euler(0f, 0f, angle), new Vector3(0.074f, 0.82f + levelValue * 0.050f, 1f), new Color(0.96f, 1f, 1f, i == 0 ? 0.50f : 0.34f), 0.58f);
            }
            SpawnEchoLink("EchoGreat_AshenConsecratedWallTop", center + (Vector3)(s * 0.46f + f * 0.34f), center - (Vector3)(s * 0.46f) + (Vector3)(f * 0.34f), new Color(1f, 0.92f, 0.62f, 0.40f), 0.46f, 0.024f);
        }

        void SpawnDualAshenParrySparks(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var count = Mathf.Clamp(3 + levelValue / 2, 4, 6);
            ...
            {
                var lane = i - (count - 1) * 0.5f;
                var pos = center + (Vector3)(s * lane * 0.10f + f * (0.04f + i * 0.018f));
                SpawnTransientSprite("EchoDual_AshenParryStar", MakeImpactDiamondSprite("EchoDual_AshenParryStar", Color.white), pos, Quaternion.Euler(0f, 0f, elapsed * -220f + i * 42f), 0.13f + levelValue * 0.007f, new Color(0.90f, 0.96f, 1f, 0.62f), 0.24f);
            }
        }

        void SpawnDualAshenEchoMutation(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            SpawnPromptSprite("EchoDual_AshenParryReturnSprite", LoadSprite(EchoAshenDualReworkPath), () => MakeAshenHolyFireMotifSprite("EchoDual_AshenParryReturnFallback", true, false), center + (Vector3)(f * 0.04f), Quaternion.Euler(0f, 0f, Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 30f), 0.92f + levelValue * 0.030f, 0.40f, new Color(1f, 1f, 1f, 0.82f), 0.32f);
            SpawnEchoWoundSlash("EchoDual_AshenParryReturnCutA", center + (Vector3)(s * 0.12f), f, new Color(1f, 0.96f, 0.72f, 0.50f), 0.58f + levelValue * 0.026f, 0.22f);
            SpawnEchoWoundSlash("EchoDual_AshenParryReturnCutB", center - (Vector3)(s * 0.12f), -f, new Color(0.86f, 0.94f, 1f, 0.46f), 0.48f + levelValue * 0.022f, 0.20f);
            SpawnEchoLink("EchoDual_AshenParryReboundThreadA", center + (Vector3)(s * 0.18f), center + (Vector3)(f * 0.42f), new Color(1f, 0.94f, 0.68f, 0.28f), 0.20f, 0.012f);
            SpawnEchoLink("EchoDual_AshenParryReboundThreadB", center - (Vector3)(s * 0.18f), center + (Vector3)(f * 0.36f), new Color(0.86f, 0.94f, 1f, 0.24f), 0.20f, 0.010f);
        }

        void SpawnGreatswordBrandCrater(Vector3 center, Vector2 forward, float radius)
        {
            var f = EchoForward(forward);
            SpawnTransientSprite("EchoGreat_OblivionCraterInner", MakeImpactDiamondSprite("EchoGreat_OblivionCraterInner", Color.white), center, Quaternion.Euler(0f, 0f, 45f + elapsed * -90f), radius * 0.34f, new Color(0.94f, 0.70f, 1f, 0.58f), 0.58f);
            SpawnGreatswordForwardWedge("EchoGreat_OblivionCraterPullCone", center - (Vector3)(f * 0.30f), f, radius * 1.35f, 64f, new Color(0.58f, 0.24f, 0.92f, 0.18f), 0.62f);
        }

        void SpawnGreatswordOblivionEchoMutation(Vector3 center, Vector2 forward, float radius, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var spike = MakeBoxSprite("EchoGreat_OblivionCraterTooth", Color.white, 12, 142);
            SpawnPromptSprite("EchoGreat_OblivionVoidCollapseSprite", LoadSprite(EchoOblivionGreatReworkPath), () => MakeOblivionBrandMotifSprite("EchoGreat_OblivionVoidCollapseFallback", true, true), center, Quaternion.Euler(0f, 0f, baseAngle - 90f), radius * 2.12f, radius * 0.86f, new Color(1f, 1f, 1f, 0.94f), 0.78f);
            for (int i = 0; i < 5; i++)
            {
                var lane = i - 2f;
                var pos = center + (Vector3)(s * lane * radius * 0.16f - f * radius * 0.10f);
                SpawnTransientSpriteScaled("EchoGreat_OblivionCraterTooth", spike, pos, Quaternion.Euler(0f, 0f, baseAngle - 90f + lane * 12f), new Vector3(0.060f, radius * (0.40f + i * 0.025f), 1f), new Color(0.78f, 0.34f, 1f, 0.48f), 0.54f);
            }
            SpawnEchoLink("EchoGreat_OblivionCollapseHorizon", center + (Vector3)(s * radius * 0.54f), center - (Vector3)(s * radius * 0.54f), new Color(0.92f, 0.54f, 1f, 0.34f), 0.46f, 0.024f);
        }

        void SpawnDualBrandHopRunes(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var count = Mathf.Clamp(3 + levelValue / 2, 4, 6);
            for (int i = 0; i < count; i++)
            {
                var t = i / Mathf.Max(1f, count - 1f);
                var pos = center + (Vector3)(f * (0.08f + t * 0.34f) + s * Mathf.Sin(i * 1.7f + elapsed) * 0.24f);
                SpawnTransientSprite("EchoDual_OblivionHopRune", MakeImpactDiamondSprite("EchoDual_OblivionHopRune", Color.white), pos, Quaternion.Euler(0f, 0f, 45f + i * 34f + elapsed * 80f), 0.16f + levelValue * 0.010f, new Color(0.88f, 0.52f, 1f, 0.64f), 0.34f);
                if (i > 0)
                {
                    var prev = center + (Vector3)(f * (0.08f + (i - 1) / Mathf.Max(1f, count - 1f) * 0.34f) + s * Mathf.Sin((i - 1) * 1.7f + elapsed) * 0.24f);
                    SpawnEchoLink("EchoDual_OblivionHopThread", prev, pos, new Color(0.82f, 0.46f, 1f, 0.26f), 0.22f, 0.010f);
                }
            }
        }

        void SpawnDualOblivionEchoMutation(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            SpawnPromptSprite("EchoDual_OblivionErasureScarsSprite", LoadSprite(EchoOblivionDualReworkPath), () => MakeOblivionBrandMotifSprite("EchoDual_OblivionErasureScarsFallback", true, false), center + (Vector3)(f * 0.04f), Quaternion.Euler(0f, 0f, Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 24f), 0.92f + levelValue * 0.030f, 0.42f, new Color(1f, 1f, 1f, 0.84f), 0.34f);
            var mark = MakeImpactDiamondSprite("EchoDual_OblivionShredMark", Color.white);
            var count = Mathf.Clamp(4 + levelValue / 2, 5, 7);
            for (int i = 0; i < count; i++)
            {
                var lane = i - (count - 1) * 0.5f;
                var pos = center + (Vector3)(s * lane * 0.09f + f * (0.05f + i * 0.026f));
                SpawnTransientSprite("EchoDual_OblivionShredMark", mark, pos, Quaternion.Euler(0f, 0f, 45f + i * 23f + elapsed * 90f), 0.11f + levelValue * 0.006f, new Color(0.90f, 0.50f, 1f, 0.54f), 0.24f + i * 0.010f);
                if (i > 0)
                {
                    var prev = center + (Vector3)(s * (lane - 1f) * 0.09f + f * (0.05f + (i - 1) * 0.026f));
                    SpawnEchoLink("EchoDual_OblivionShredThread", prev, pos, new Color(0.84f, 0.44f, 1f, 0.24f), 0.18f, 0.008f);
                }
            }
        }

        void SpawnAshenMemoryGuardPlate(Vector3 center, Vector2 forward, int levelValue)
        {
            var f = EchoForward(forward);
            var angle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 90f;
            var plate = MakeBoxSprite("MemoryAshenGuardPlate", Color.white, 28, 128);
            ...
            for (int i = 0; i < shardCount; i++)
            {
                var a = angle + 90f + i * (360f / shardCount) + Mathf.Sin(i * 1.7f) * 10f;
                var dir = Quaternion.Euler(0f, 0f, a) * Vector3.right;
                SpawnTransientSprite("MemoryAshenGuardPlateShard", shard, center + dir * radius * 0.38f, Quaternion.Euler(0f, 0f, a + 45f), 0.12f + levelValue * 0.006f, new Color(0.92f, 0.98f, 1f, 0.50f), 0.30f + i * 0.010f);
            }
        }

        void SpawnAshenHolyFlames(Vector3 center, Vector2 forward, int levelValue, bool echo, bool strong)
        {
            var f = EchoForward(forward);
            var s = new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var prefix = echo ? "EchoAshenHolyFlame" : "MemoryAshenHolyFlame";
            var tongue = MakeBoxSprite($"{prefix}_Tongue", Color.white, strong ? 13 : 10, strong ? 142 : 108);
            var ember = MakeImpactDiamondSprite($"{prefix}_Ember", Color.white);
            var ash = MakeBoxSprite($"{prefix}_AshSmoke", Color.white, 22, 82);
            var flameCount = Mathf.Clamp((strong ? 6 : 4) + levelValue / 2, strong ? 7 : 4, strong ? 10 : 7);
            var span = strong ? 0.92f + levelValue * 0.040f : 0.56f + levelValue * 0.030f;
            var lifetime = strong ? 0.62f : 0.36f;
            var gold = strong ? new Color(1f, 0.88f, 0.48f, 0.72f) : new Color(1f, 0.90f, 0.62f, 0.54f);
            ...
            {
                var lane = i - (flameCount - 1) * 0.5f;
                var offset = s * lane * 0.12f * span + f * (0.08f + Mathf.Abs(lane) * 0.018f);
                var angle = baseAngle - 90f + lane * (strong ? 8f : 10f) + Mathf.Sin(elapsed * 3f + i) * 8f;
                var height = (0.42f + levelValue * 0.022f) * (strong ? 1.36f : 1f) * (1f - Mathf.Abs(lane) / Mathf.Max(2.6f, flameCount));
                var color = i % 2 == 0 ? gold : white;
                SpawnTransientSpriteScaled($"{prefix}_RisingTongue", tongue, center + (Vector3)offset + Vector3.up * (0.02f + i * 0.006f), Quaternion.Euler(0f, 0f, angle), new Vector3(strong ? 0.046f : 0.034f, height, 1f), color, lifetime * (0.72f + i * 0.018f));
            }

            var emberCount = strong ? 7 : 4;
            for (int i = 0; i < emberCount; i++)
            {
                var lane = i - (emberCount - 1) * 0.5f;
                var pos = center + (Vector3)(s * lane * 0.11f * span + f * (0.22f + (i % 3) * 0.045f));
                SpawnTransientSprite($"{prefix}_GoldAshEmber", ember, pos, Quaternion.Euler(0f, 0f, 45f + baseAngle + i * 29f), strong ? 0.13f : 0.09f, new Color(1f, 0.92f, 0.58f, strong ? 0.66f : 0.46f), lifetime * 0.55f);
            }
        }

        void SpawnAshenEchoCounterBurst(Vector3 center, Vector2 forward, int levelValue, bool heavy, bool dense)
        {
            var f = EchoForward(forward);
            var angle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg - 90f;
            var radius = heavy ? 1.18f + levelValue * 0.08f : 0.62f + levelValue * 0.045f;
            if (dense) radius *= 0.80f;
            SpawnAshenHolyFlames(center, f, levelValue, true, heavy);
            SpawnRadialSlashLines(heavy ? "EchoGreat_AshenPlateBreakShard" : "EchoDual_AshenParryBreakShard", center, f, dense ? 3 : heavy ? 8 : 5, radius * (heavy ? 0.78f : 0.56f), new Color(0.96f, 1f, 1f, heavy ? 0.66f : 0.48f), heavy ? 0.48f : 0.28f);
            if (heavy)
            {
                SpawnGreatswordForwardWedge("EchoGreat_AshenCounterShockFront", center - (Vector3)(f * 0.22f), f, 1.92f + levelValue * 0.10f, 44f, new Color(0.92f, 0.98f, 1f, 0.24f), 0.52f);
                SpawnEchoWoundSlash("EchoGreat_AshenCounterBreakLine", center + (Vector3)(f * 0.18f), f, new Color(0.96f, 1f, 1f, 0.70f), 1.12f + levelValue * 0.06f, 0.42f);
            }
            else if (!dense)
            {
                var s = new Vector2(-f.y, f.x);
                SpawnEchoWoundSlash("EchoDual_AshenCounterCutA", center + (Vector3)(s * 0.10f), f, new Color(0.96f, 1f, 1f, 0.66f), 0.68f + levelValue * 0.035f, 0.24f);
                SpawnEchoWoundSlash("EchoDual_AshenCounterCutB", center - (Vector3)(s * 0.10f), -f, new Color(0.82f, 0.92f, 1f, 0.54f), 0.54f + levelValue * 0.025f, 0.22f);
            }
            SpawnTransientSprite(heavy ? "EchoGreat_AshenCounterCoreFlash" : "EchoDual_AshenCounterCoreFlash", MakeImpactDiamondSprite("EchoAshenCounterCoreFlash", Color.white), center, Quaternion.Euler(0f, 0f, angle + 45f), heavy ? 0.34f : 0.20f, new Color(1f, 1f, 1f, heavy ? 0.78f : 0.58f), heavy ? 0.24f : 0.16f);
        }

        void SpawnOblivionEraseBurst(Vector3 center, Vector2 forward, float radius, bool heavy, string name)
        {
            var f = EchoForward(forward);
            ...
            if (name.StartsWith("EchoGreat_", StringComparison.Ordinal))
            {
                SpawnPromptSprite($"{name}_VoidCollapseSprite", LoadSprite(EchoOblivionGreatReworkPath), () => MakeOblivionBrandMotifSprite($"{name}_VoidCollapseFallback", true, true), center, Quaternion.Euler(0f, 0f, baseAngle - 90f), radius * 2.08f, motifScale, new Color(1f, 1f, 1f, 0.86f), heavy ? 0.62f : 0.38f);
            }
            else if (name.StartsWith("EchoDual_", StringComparison.Ordinal))
            {
                SpawnPromptSprite($"{name}_ErasureScarsSprite", LoadSprite(EchoOblivionDualReworkPath), () => MakeOblivionBrandMotifSprite($"{name}_ErasureScarsFallback", true, false), center + (Vector3)(f * 0.05f), Quaternion.Euler(0f, 0f, baseAngle - 28f), radius * 1.24f, motifScale, new Color(1f, 1f, 1f, 0.78f), heavy ? 0.62f : 0.38f);
            }
            else
            {
            ...
            SpawnTransientSprite($"{name}_VoidCoreShard", MakeImpactDiamondSprite($"{name}_VoidCoreShard", Color.white), center, Quaternion.Euler(0f, 0f, 45f + elapsed * -100f), radius * (heavy ? 0.34f : 0.24f), voidColor, heavy ? 0.62f : 0.38f);
            ...
            SpawnRadialSlashLines($"{name}_VoidCrack", center, f, heavy ? 9 : 5, radius * (heavy ? 0.82f : 0.56f), new Color(0.90f, 0.52f, 1f, heavy ? 0.70f : 0.54f), heavy ? 0.54f : 0.30f);
            var shard = MakeImpactDiamondSprite($"{name}_VoidFragment", Color.white);
            for (int i = 0; i < (heavy ? 8 : 4); i++)
            {
                var angle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg + i * (heavy ? 45f : 90f) + Mathf.Sin(i * 1.43f) * 14f;
                var dir = Quaternion.Euler(0f, 0f, angle) * Vector3.right;
                SpawnTransientSprite($"{name}_LiftedFragment", shard, center + dir * radius * (0.28f + i * 0.018f), Quaternion.Euler(0f, 0f, angle + 45f), heavy ? 0.18f : 0.12f, new Color(0.86f, 0.44f, 1f, heavy ? 0.62f : 0.42f), heavy ? 0.44f : 0.26f);
            }
        }

        void SpawnUltimateDopamineBurst(string name, Vector3 center, Vector2 forward, float radius, Color primary, Color secondary, int shardCount, float lifetime)
        {
            var f = EchoForward(forward);
            SpawnTransientSprite($"{name}_OuterShockRing", MakeRingSprite($"{name}_OuterShockRing", Color.white, 192), center, Quaternion.Euler(0f, 0f, elapsed * -100f), radius, new Color(primary.r, primary.g, primary.b, primary.a * 0.68f), lifetime);
            SpawnTransientSprite($"{name}_WhiteHotCore", MakeDiscSprite($"{name}_WhiteHotCore", Color.white, 128), center, Quaternion.identity, radius * 0.18f, new Color(secondary.r, secondary.g, secondary.b, secondary.a * 0.72f), lifetime * 0.62f);
            SpawnRadialSlashLines($"{name}_ImpactSpokes", center, f, Mathf.Clamp(shardCount / 2, 5, 10), radius * 0.52f, secondary, lifetime * 0.72f);
            var shard = MakeBoxSprite($"{name}_BladeShard", Color.white, 7, 118);
            for (int i = 0; i < shardCount; i++)
            {
                var angle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg + i * (360f / shardCount) + Mathf.Sin(i * 1.37f) * 8f;
                var dir = Quaternion.Euler(0f, 0f, angle) * Vector3.right;
                var pos = center + dir * radius * (0.45f + (i % 3) * 0.055f);
                var color = i % 2 == 0 ? secondary : primary;
                SpawnTransientSpriteScaled($"{name}_OrbitShard", shard, pos, Quaternion.Euler(0f, 0f, angle + 92f), new Vector3(0.030f, radius * 0.18f, 1f), color, lifetime * (0.70f + i * 0.010f));
            }
        }
        ...
        void SpawnUltimateClockBurst(string name, Vector3 center, Vector2 forward, float radius, bool heavy)
        {
            var f = EchoForward(forward);
            var gold = new Color(1f, 0.82f, 0.24f, heavy ? 0.82f : 0.66f);
            var ice = new Color(0.86f, 0.92f, 1f, heavy ? 0.62f : 0.46f);
            SpawnStoppedSecondField(center, radius, gold, heavy ? 1.80f : 1.32f, true);
            SpawnUltimateDopamineBurst(name, center, forward, radius * 0.92f, gold, ice, heavy ? 12 : 8, heavy ? 0.66f : 0.46f);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var hand = MakeBoxSprite($"{name}_SecondHand", Color.white, heavy ? 8 : 6, heavy ? 190 : 132);
            SpawnClockHandSweep($"{name}_ClockHandSnap", hand, center, radius, baseAngle - 110f, baseAngle + 250f, heavy ? 0.052f : 0.040f, heavy ? 0.72f : 0.62f, gold, heavy ? 1.15f : 0.86f, heavy ? 0.72f : 0.52f);
        }

        void SpawnAshenOblivionRupture(Vector3 center, Vector2 forward, float radius, bool heavy, string name)
        {
            var f = EchoForward(forward);
        ...
        void SpawnKalmuriHungerEchoVfx(Vector3 origin, Vector2 forward, Vector2 side, List<V1Enemy> targets, int level, int hitIndex, bool isHeavy, bool denseDualBlade, bool playSound)
        {
            ...
            if (denseDualBlade && !isHeavy)
            {
                SpawnEchoWoundSlash("KalmuriHunger_DualDenseRift", origin, f, new Color(dualMawEdge.r, dualMawEdge.g, dualMawEdge.b, 0.58f), 0.86f, 0.34f);
                SpawnTransientSprite("KalmuriHunger_DualDenseCore", hungerBlade, origin, Quaternion.Euler(0f, 0f, baseAngle + 62f + hitIndex * 11f), 0.26f, new Color(dualMawViolet.r, dualMawViolet.g, dualMawViolet.b, 0.68f), 0.36f);
                return;
            }

            if (!denseDualBlade && isHeavy)
            {
                SpawnTransientSprite("KalmuriHunger_GreatMemoryPool", MakeDiscSprite("KalmuriHungerMemoryPool", Color.white, 144), origin, Quaternion.identity, radius * 0.62f, new Color(memoryPool.r, memoryPool.g, memoryPool.b, memoryPool.a + 0.08f), 1.05f);
            ...
            var trailCount = denseDualBlade ? (isHeavy ? 3 : 4) : isHeavy ? 7 : 11;
            for (int i = 0; i < trailCount; i++)
            {
                ...
                var start = origin - (Vector3)(f * (isHeavy ? 1.36f : 0.82f) - s * (spread + jitter));
            ...
            if (isHeavy)
            {
                ...
                SpawnSweepingTransientSprite("KalmuriHunger_GreatUpperBladeJaw", hungerBlade, origin + (Vector3)(s * 0.92f), origin + (Vector3)(s * 0.22f), baseAngle + 112f, baseAngle + 142f, 0.52f, 0.32f, hot, 1.10f, 0.52f);
                SpawnSweepingTransientSprite("KalmuriHunger_GreatLowerBladeJaw", hungerBlade, origin - (Vector3)(s * 0.92f), origin - (Vector3)(s * 0.22f), baseAngle - 112f, baseAngle - 142f, 0.52f, 0.32f, hot, 1.10f, 0.52f);
                SpawnSweepingTransientSprite("KalmuriHunger_GreatJawAfterimage", jaw, origin, origin + (Vector3)(f * 0.12f), baseAngle + 4f, baseAngle - 22f, 0.44f, 0.24f, new Color(0.42f, 0.88f, 1f, 0.42f), 0.82f, 0.42f);
                SpawnRadialSlashLines("KalmuriHunger_GreatBlueSplinterScars", origin, f, 6, 1.34f, new Color(0.62f, 1f, 1f, 0.60f), 0.92f);
            }
            else
            {
        ...
        float AngleForBladePull(float baseAngle, int index, bool isHeavy)
        {
            return baseAngle + 58f + index * (isHeavy ? 7f : 11f);
        }
        ...
        void ResolveKalmuriPrototypeFollowup(int concept, Vector3 origin, Vector2 forward, int level, int hitIndex, WeaponRuntimeSpec weapon, bool denseDualBlade)
        {
            ...
            switch (clamped)
            {
                ...
                case 2:
                    ...
                    DealKalmuriPrototypeDamage(targets, origin, f, baseDamage * (isHeavy ? 1.10f : 0.92f), isHeavy ? 0.56f : 0.34f, "Kalmuri Echo K2 Blood Scent");
                    break;
                case 3:
                    DebugPreviewKalmuriCrossSwarm(origin, f, s, targets, isHeavy, false);
                    DealKalmuriPrototypeDamage(targets, origin, f, baseDamage * (isHeavy ? 1.28f : 1.02f), isHeavy ? 0.78f : 0.48f, "Kalmuri Echo K3 Feast Table");
                    break;
                default:
                    DebugPreviewKalmuriMarkFrenzy(origin, f, s, targets, isHeavy, false);
                    DealKalmuriPrototypeDamage(targets, origin, f, baseDamage * (isHeavy ? 1.04f : 0.88f), isHeavy ? 0.50f : 0.30f, "Kalmuri Echo K4 Chewed Trail");
                    break;
            }
        ...
        float KalmuriPrototypeScore(int concept, float x, float y, float dist, float touchRadius, int level, bool isHeavy, float areaMul)
        {
            ...
            switch (concept)
            {
                ...
                case 2:
                {
                    var radius = (isHeavy ? 1.42f : 1.14f) * areaMul + radiusBonus;
                    if (dist > radius) return -1f;
                    return radius - dist + Mathf.Max(0f, x) * 0.22f;
                }
                case 3:
                {
                    var radius = (isHeavy ? 1.22f : 0.96f) * areaMul + radiusBonus;
                    if (dist > radius) return -1f;
                    return radius - dist + Mathf.Max(0f, 0.34f - Mathf.Abs(dist - radius * 0.48f)) * 1.4f;
                }
                default:
                {
                    var length = (isHeavy ? 1.72f : 1.28f) * areaMul + radiusBonus;
                    var halfWidth = (isHeavy ? 0.58f : 0.46f) * areaMul + radiusBonus;
                    if (x < -0.62f || x > length || Mathf.Abs(y) > halfWidth) return -1f;
                    return length - Mathf.Abs(x) * 0.35f + (halfWidth - Mathf.Abs(y)) * 1.6f;
                }
            }
        }
        ...
        void SpawnBloodStormOpening(bool heavy)
        {
            ...
            SpawnUltimateDopamineBurst("Ult_BloodBladeStormOpening", player.position, lastAim, heavy ? 3.45f : 2.86f, new Color(1f, 0.06f, 0.11f, 0.72f), new Color(1f, 0.96f, 0.92f, 0.74f), heavy ? 15 : 11, heavy ? 0.72f : 0.58f);
            ...
            hitstopTimer = Mathf.Max(hitstopTimer, heavy ? 0.075f : 0.052f);
        ...
        void BloodStormBurst(bool heavy)
        {
            ...
            SpawnUltimateDopamineBurst("Ult_BloodBladeStormClimax", center, lastAim, heavy ? 3.20f : 2.70f, new Color(1f, 0.02f, 0.08f, heavy ? 0.80f : 0.66f), new Color(1f, 0.96f, 0.94f, heavy ? 0.82f : 0.70f), heavy ? 18 : 13, heavy ? 0.54f : 0.40f);
        ...
        void SpawnKalmuriAwakenHungerBloom(V1Enemy first, Vector2 forward, WeaponRuntimeSpec weapon, int level)
        {
            ...
            var hot = isHeavy ? new Color(0.78f, 1f, 1f, 0.94f) : new Color(0.56f, 0.96f, 1f, 0.84f);
            var blue = isHeavy ? new Color(0.16f, 0.70f, 1f, 0.58f) : new Color(0.20f, 0.86f, 1f, 0.42f);

            PlaySfx("kalmuri_lunge", isHeavy ? 0.84f : 0.68f, 0.07f);
            if (isHeavy)
            {
                SpawnTransientSprite("KalmuriAwaken_GreatMemoryWakeRing", ring, origin, Quaternion.identity, radius * 0.88f, blue, 0.46f);
            }
            SpawnEchoWoundSlash(isHeavy ? "KalmuriAwaken_GreatBlueTear" : "KalmuriAwaken_DualBlueTear", origin, f, new Color(0.62f, 1f, 1f, isHeavy ? 0.86f : 0.64f), isHeavy ? 1.34f : 0.78f, isHeavy ? 0.50f : 0.34f);
            ...
            for (int i = 0; i < biteCount; i++)
            {
                ...
                SpawnSweepingTransientSprite(isHeavy ? "KalmuriAwaken_GreatMemoryBladeDevour" : "KalmuriAwaken_DualMemoryBladeDevour", hungerBlade, start, end, angle + 62f, angle + 188f, isHeavy ? 0.22f : 0.14f, isHeavy ? 0.11f : 0.07f, hot, isHeavy ? 0.48f : 0.34f, isHeavy ? 0.18f : 0.12f);
            }
            ...
            for (int i = 0; i < targets.Count; i++)
            {
                ...
                if (i > 0)
                {
                    SpawnEchoLink(isHeavy ? "KalmuriAwaken_GreatBlueScentChain" : "KalmuriAwaken_DualBlueScentChain", targets[i].transform.position, origin, new Color(0.34f, 0.92f, 1f, isHeavy ? 0.42f : 0.30f), isHeavy ? 0.42f : 0.30f, isHeavy ? 0.026f : 0.016f);
                }
        ...
        void DebugPreviewKalmuriConcept(int concept)
        {
            ...
            var f = lastAim.sqrMagnitude > 0.01f ? lastAim.normalized : Vector2.up;
            var s = new Vector2(-f.y, f.x).normalized;
            var targets = DebugBuildKalmuriPreviewPack(f, s);
            if (targets.Count == 0) return;
            ...
            switch (debugKalmuriEchoConcept)
            {
                case 1:
                    ...
                    SpawnFloatingText(origin + Vector3.up * 0.62f, heavy ? "K1 GREAT Wound Maw / PLAYABLE" : "K1 DUAL Bite Swarm / PLAYABLE", new Color(1f, 0.54f, 0.34f));
                    Log("Debug Kalmuri playable concept 1 wound feast");
                    break;
                case 2:
                    DebugPreviewKalmuriTrailBloom(origin, f, s, targets, heavy);
                    SpawnFloatingText(origin + Vector3.up * 0.62f, heavy ? "K2 GREAT Blood Scent / PLAYABLE" : "K2 DUAL Scent Pack / PLAYABLE", new Color(1f, 0.38f, 0.24f));
                    Log("Debug Kalmuri playable concept 2 blood scent hunt");
                    break;
                case 3:
                    DebugPreviewKalmuriCrossSwarm(origin, f, s, targets, heavy);
                    SpawnFloatingText(origin + Vector3.up * 0.62f, heavy ? "K3 GREAT Feast Table / PLAYABLE" : "K3 DUAL Feast Ring / PLAYABLE", new Color(1f, 0.66f, 0.36f));
                    Log("Debug Kalmuri playable concept 3 feast table");
                    break;
                default:
                    DebugPreviewKalmuriMarkFrenzy(origin, f, s, targets, heavy);
                    SpawnFloatingText(origin + Vector3.up * 0.62f, heavy ? "K4 GREAT Chewed Furrow / PLAYABLE" : "K4 DUAL Bite Trail / PLAYABLE", new Color(1f, 0.48f, 0.28f));
                    Log("Debug Kalmuri playable concept 4 chewed trail");
                    break;
            }
        ...
        List<V1Enemy> DebugBuildKalmuriPreviewPack(Vector2 forward, Vector2 side)
        {
            ...
            {
                Vector2.zero,
                side * 0.52f + forward * 0.24f,
                -side * 0.52f + forward * 0.18f,
                forward * 0.78f,
                -forward * 0.46f + side * 0.22f,
            };
            var kinds = new[]
            {
                V1EnemyKind.Eroder,
                V1EnemyKind.DriftingEye,
                V1EnemyKind.SplitOne,
                V1EnemyKind.Eroder,
                V1EnemyKind.VoidPriest,
            };
            ...
            SpawnFloatingText(center + Vector3.up * 0.92f, "Kalmuri Echo Hunger Prototypes / HP 9999", new Color(1f, 0.92f, 0.52f));
            return result;
        }
        ...
        void DebugPreviewKalmuriTrailBloom(Vector3 origin, Vector2 forward, Vector2 side, List<V1Enemy> targets, bool heavy, bool previewDamage = true)
        {
            ...
            var s = side.sqrMagnitude > 0.01f ? side.normalized : new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var tooth = MakeImpactDiamondSprite("KalmuriBloodScent_Tooth", Color.white);
            var scar = MakeBoxSprite("KalmuriBloodScent_Scar", Color.white, 8, 128);
            var blood = heavy ? new Color(1f, 0.19f, 0.08f, 0.72f) : new Color(1f, 0.12f, 0.06f, 0.58f);
            ...
            if (heavy)
            {
                for (int i = 0; i < 5; i++)
                {
                    var offset = (i - 2) * 0.16f;
                    var start = origin - (Vector3)(f * (0.90f + i * 0.05f) - s * offset);
                    var end = origin + (Vector3)(f * 0.14f + s * offset * 0.18f);
                    SpawnSweepingTransientSprite("KalmuriBloodScent_GreatDrawnToWound", scar, start, end, baseAngle - 90f + i * 5f, baseAngle - 116f + i * 6f, 0.24f, 0.16f, hot, 0.78f, 0.28f);
                }
                SpawnEchoWoundSlash("KalmuriBloodScent_GreatDevourScar", origin + (Vector3)(f * 0.08f), f, new Color(1f, 0.22f, 0.08f, 0.88f), 1.72f, 0.74f);
            }
            else
            {
                const int pack = 12;
                ...
                {
                    var angle = baseAngle + i * 31f + Mathf.Sin(i * 0.77f) * 18f;
                    var dir = (Vector2)(Quaternion.Euler(0f, 0f, angle) * Vector3.right);
                    var start = origin + (Vector3)(dir * (0.82f + (i % 3) * 0.08f));
                    var end = origin + (Vector3)(dir * 0.10f + s * Mathf.Sin(i * 1.33f) * 0.05f);
                    SpawnSweepingTransientSprite("KalmuriBloodScent_DualPackBite", tooth, start, end, angle + 45f, angle + 162f, 0.15f, 0.08f, hot, 0.66f, 0.16f);
                }
            }
            for (int i = 0; i < linked.Count; i++)
            {
        ...
        void DebugPreviewKalmuriCrossSwarm(Vector3 origin, Vector2 forward, Vector2 side, List<V1Enemy> targets, bool heavy, bool previewDamage = true)
        {
            ...
            var s = side.sqrMagnitude > 0.01f ? side.normalized : new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var radius = heavy ? 1.12f : 0.86f;
            var tooth = MakeImpactDiamondSprite("KalmuriFeastTable_Tooth", Color.white);
            var skewer = MakeBoxSprite("KalmuriFeastTable_Skewer", Color.white, 8, 136);
            ...
            if (heavy)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    var dir = (Vector2)(Quaternion.Euler(0f, 0f, angle) * Vector3.right);
                    var start = origin + (Vector3)(dir * radius);
                    var end = origin + (Vector3)(dir * 0.28f);
                    SpawnSweepingTransientSprite("KalmuriFeastTable_GreatSkewerClose", skewer, start, end, angle - 90f, angle - 126f, 0.36f, 0.22f, hot, 0.86f, 0.30f);
                }
                SpawnEchoWoundSlash("KalmuriFeastTable_GreatFinalCarve", origin, f, new Color(1f, 0.18f, 0.06f, 0.90f), 1.76f, 0.72f);
                SpawnRadialSlashLines("KalmuriFeastTable_GreatChewMarks", origin, f, 8, 1.04f, new Color(1f, 0.52f, 0.18f, 0.50f), 0.74f);
            }
            else
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    var dir = (Vector2)(Quaternion.Euler(0f, 0f, angle) * Vector3.right);
                    var start = origin + (Vector3)(dir * (radius + (i % 2) * 0.08f));
                    var end = origin + (Vector3)(dir * 0.18f);
                    SpawnSweepingTransientSprite("KalmuriFeastTable_DualTeethFold", tooth, start, end, angle + 45f, angle + 184f, 0.16f, 0.08f, hot, 0.70f, 0.18f);
                }
                SpawnRadialSlashLines("KalmuriFeastTable_DualChewMarks", origin, f, 12, 0.78f, new Color(1f, 0.26f, 0.10f, 0.52f), 0.62f);
            }
            if (previewDamage) DebugDamageKalmuriPreview(targets, origin, heavy ? 1.28f : 0.98f, heavy ? 42f : 26f, "Kalmuri feast table");
        }
        ...
        void DebugPreviewKalmuriMarkFrenzy(Vector3 origin, Vector2 forward, Vector2 side, List<V1Enemy> targets, bool heavy, bool previewDamage = true)
        {
            ...
            var s = side.sqrMagnitude > 0.01f ? side.normalized : new Vector2(-f.y, f.x);
            var baseAngle = Mathf.Atan2(f.y, f.x) * Mathf.Rad2Deg;
            var tooth = MakeImpactDiamondSprite("KalmuriChewedTrail_Tooth", Color.white);
            var line = MakeBoxSprite("KalmuriChewedTrail_Gash", Color.white, 9, 150);
            var hot = heavy ? new Color(1f, 0.68f, 0.26f, 0.92f) : new Color(1f, 0.42f, 0.20f, 0.74f);

            PlaySfx(heavy ? "kalmuri_echo_heavy" : "kalmuri_echo", heavy ? 0.84f : 0.68f, 0.05f);
            if (heavy)
            {
                var trenchStart = origin - (Vector3)(f * 0.90f);
                var trenchEnd = origin + (Vector3)(f * 0.92f);
                SpawnEchoLink("KalmuriChewedTrail_GreatFurrowBlood", trenchStart, trenchEnd, new Color(1f, 0.16f, 0.06f, 0.56f), 0.82f, 0.056f);
                SpawnTransientSpriteScaled("KalmuriChewedTrail_GreatFurrowScar", line, origin, Quaternion.Euler(0f, 0f, baseAngle - 90f), new Vector3(0.066f, 1.46f, 1f), new Color(1f, 0.22f, 0.08f, 0.72f), 0.82f);
                for (int i = 0; i < 4; i++)
                {
                    var t = i / 3f - 0.5f;
                    var pos = origin + (Vector3)(f * (t * 1.30f) + s * Mathf.Sin(i * 1.41f) * 0.09f);
                    SpawnTransientSprite("KalmuriChewedTrail_GreatBiteSocket", MakeRingSprite("KalmuriChewedTrail_GreatBiteSocket", Color.white, 104), pos, Quaternion.Euler(0f, 0f, baseAngle), 0.42f, new Color(1f, 0.25f, 0.08f, 0.42f), 0.68f);
                    SpawnRadialSlashLines("KalmuriChewedTrail_GreatBiteTeeth", pos, f, 5, 0.44f, hot, 0.58f);
                }
            }
            else
            {
                ...
                SpawnEchoWoundSlash("KalmuriChewedTrail_DualFinalChew", origin + (Vector3)(f * 0.14f), f, new Color(1f, 0.16f, 0.06f, 0.62f), 1.20f, 0.62f);
            }
            if (previewDamage) DebugDamageKalmuriPreview(targets, origin, heavy ? 1.36f : 1.02f, heavy ? 34f : 22f, "Kalmuri chewed trail");
        }
        ...
        void DebugDamageKalmuriPreview(List<V1Enemy> targets, Vector3 origin, float radius, float damage, string source)
        {
            ...
            cameraShakeAmount = Mathf.Max(cameraShakeAmount, 0.035f);
        }
            ...
            try
            {
                for (int cycle = 0; cycle < 3; cycle++)
                {
                    ...
                    for (int i = 0; i < hits.Count; i++)
                    {
                        ...
                        {
                            DealDamage(hit.Enemy, weapon.Damage * 0.08f, weapon.DisplayName, true, hit.Dir.normalized, weapon.PrimaryKnock);
        ...
        void DrawHud()
        {
            ...
            GUI.Box(new Rect(Screen.width - 326, 12, 314, 468), "", panelStyle);
        ...
        void SpawnHeldPromptSprite(string name, Sprite promptSprite, Func<Sprite> fallbackSprite, Vector3 position, Quaternion rotation, float targetWorldWidth, float fallbackScale, Color color, float lifetime, float holdRatio)
        {
            var sprite = promptSprite;
            var scale = fallbackScale;
            if (sprite != null)
        ...
        GameObject SpawnHeldTransientSprite(string name, Sprite sprite, Vector3 position, Quaternion rotation, float scale, Color color, float lifetime, float holdRatio)
        {
            return SpawnHeldTransientSpriteScaled(name, sprite, position, rotation, Vector3.one * scale, color, lifetime, holdRatio);
        }

        GameObject SpawnHeldTransientSpriteScaled(string name, Sprite sprite, Vector3 position, Quaternion rotation, Vector3 scale, Color color, float lifetime, float holdRatio)
        {
            var go = SpawnTransientSpriteScaled(name, sprite, position, rotation, scale, color, lifetime);
            var fading = go.GetComponent<V1FadingSprite>();
            if (fading != null) fading.Configure(this, lifetime, holdRatio);
            return go;
        }
        ...
        {
            SpawnTransientSprite(name, KalmuriBladeSprite(), position, Quaternion.Euler(0f, 0f, angle), scale * KalmuriEchoVisibilityScale, color, lifetime);
        }
        ...
        void SpawnKalmuriDiveBlade(string name, Vector3 start, Vector3 end, float scale, Color color, float lifetime, float sweepDuration)
        {
            ...
            SpawnEchoLink($"{name}_Trail", start, end, new Color(color.r, color.g, color.b, color.a * 0.54f), Mathf.Min(lifetime, sweepDuration + 0.10f), 0.024f + scale * 0.042f);
            SpawnSweepingTransientSprite(
                ...
                angle + 58f,
                angle + 74f,
                scale * KalmuriEchoVisibilityScale,
                scale * KalmuriEchoVisibilityScale * 1.28f,
                color,
                lifetime,
                sweepDuration);
        }
        ...
        void SpawnStoppedSecondField(Vector3 center, float radius, Color color, float lifetime, bool strong)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                SpawnHeldTransientSpriteScaled("StoppedSecondClockTick", tick, pos, Quaternion.Euler(0f, 0f, angle), tickScale, new Color(color.r, color.g, color.b, ringAlpha), lifetime, 0.82f);
            }
        ...
        void SpawnClockHands(Vector3 center, float radius, Color color, float lifetime)
        {
            ...
            SpawnClockHandSweep("ClockHand_Long", longHand, center, radius, baseAngle, baseAngle + 360f, 0.046f, 0.64f, color, lifetime, Mathf.Max(0.8f, lifetime * 0.74f));
            SpawnClockHandStatic("ClockHand_Short", shortHand, center, radius, baseAngle + 86f, 0.040f, 0.46f, new Color(color.r, color.g, color.b, color.a * 0.86f), lifetime);
            SpawnHeldTransientSprite("ClockHand_Core", MakeImpactDiamondSprite("ClockHand_Core", Color.white), center, Quaternion.Euler(0f, 0f, baseAngle + 45f), 0.22f, new Color(color.r, color.g, color.b, Mathf.Min(0.92f, color.a + 0.12f)), lifetime, 0.82f);
        }

        void SpawnClockHandSweep(string name, Sprite sprite, Vector3 center, float radius, float startAngle, float endAngle, float width, float lengthMul, Color color, float lifetime, float duration)
        {
            var length = radius * lengthMul;
            var dir = Quaternion.Euler(0f, 0f, startAngle) * Vector3.up;
            var go = SpawnHeldTransientSpriteScaled(name, sprite, center + dir * (length * 0.50f), Quaternion.Euler(0f, 0f, startAngle), new Vector3(width, length, 1f), color, lifetime, 0.82f);
            var sweep = go.GetComponent<V1ClockHandSweep>();
            if (sweep == null) sweep = go.AddComponent<V1ClockHandSweep>();
            sweep.Configure(center, length * 0.50f, startAngle, endAngle, duration);
        }

        void SpawnClockHandStatic(string name, Sprite sprite, Vector3 center, float radius, float angle, float width, float lengthMul, Color color, float lifetime)
        {
            var length = radius * lengthMul;
            var dir = Quaternion.Euler(0f, 0f, angle) * Vector3.up;
            SpawnHeldTransientSpriteScaled(name, sprite, center + dir * (length * 0.50f), Quaternion.Euler(0f, 0f, angle), new Vector3(width, length, 1f), color, lifetime, 0.82f);
        }
        ...
        GameObject SpawnTransientSpriteScaled(string name, Sprite sprite, Vector3 position, Quaternion rotation, Vector3 scale, Color color, float lifetime)
        {
            ...
            sr.color = color;
            sr.sortingOrder = TransientVfxSortingOrder(name);
            var fading = go.GetComponent<V1FadingSprite>();
        ...
        int TransientVfxSortingOrder(string name)
        {
            if (name.Contains("StoppedSecond", StringComparison.OrdinalIgnoreCase)) return 12;
            if (name.Contains("StoppedEcho", StringComparison.OrdinalIgnoreCase)) return 12;
            if (name.Contains("ClockHand", StringComparison.OrdinalIgnoreCase)) return 12;
            if (name.Contains("StoppedDome", StringComparison.OrdinalIgnoreCase)) return 12;
            if (name.Contains("StasisHuntField", StringComparison.OrdinalIgnoreCase)) return 12;
            if (name.Contains("StasisHuntDome", StringComparison.OrdinalIgnoreCase)) return 12;
            if (name.Contains("StasisHuntClock", StringComparison.OrdinalIgnoreCase)) return 12;
            if (name.Contains("Kalmuri", StringComparison.OrdinalIgnoreCase)) return 54;
            if (name.Contains("EchoGreat", StringComparison.OrdinalIgnoreCase)) return 52;
            if (name.Contains("EchoDual", StringComparison.OrdinalIgnoreCase)) return 51;
            if (name.Contains("Echo", StringComparison.OrdinalIgnoreCase)) return 50;
            if (name.Contains("Ult", StringComparison.OrdinalIgnoreCase)) return 50;
            return 40;
        }
        ...
        public void ReleaseTransientSprite(V1FadingSprite fading)
        {
            ...
            var clockHand = go.GetComponent<V1ClockHandSweep>();
        ...
        static Sprite MakeBloodVortexRingSprite(string name, Color color, bool sharp)
        {
            return CachedGeneratedSprite($"bloodVortex:{name}:{ColorKey(color)}:{sharp}", () => MakeBloodVortexRingSpriteUncached(name, color, sharp));
        }

        static Sprite MakeBloodVortexRingSpriteUncached(string name, Color color, bool sharp)
        {
            const int size = 256;
            var tex = new Texture2D(size, size, TextureFormat.RGBA32, false) { name = name };
            var center = new Vector2(size * 0.5f, size * 0.5f);
            var ringRadius = sharp ? size * 0.395f : size * 0.360f;
            for (int y = 0; y < size; y++)
            for (int x = 0; x < size; x++)
            {
                var p = new Vector2(x, y) - center;
                var d = p.magnitude;
                var angle = Mathf.Atan2(p.y, p.x) * Mathf.Rad2Deg;
                var normalizedAngle = Mathf.Repeat(angle + 360f, 360f);
                var swirl = Mathf.Sin((normalizedAngle * (sharp ? 3.1f : 2.4f) + d * 0.42f) * Mathf.Deg2Rad);
                var broken = Mathf.Clamp01((swirl + (sharp ? 0.36f : 0.18f)) * (sharp ? 1.40f : 1.10f));
                var edgeNoise = Mathf.Sin((normalizedAngle * 7.0f - d * 0.18f) * Mathf.Deg2Rad) * (sharp ? 4.2f : 3.0f);
                var mainBand = Mathf.Clamp01(1f - Mathf.Abs(d - ringRadius - edgeNoise) / (sharp ? 5.2f : 8.0f));
                var innerBand = Mathf.Clamp01(1f - Mathf.Abs(d - ringRadius * 0.88f + edgeNoise * 0.45f) / (sharp ? 3.2f : 5.6f)) * (sharp ? 0.36f : 0.58f);
                var outerBleed = Mathf.Clamp01(1f - Mathf.Abs(d - ringRadius * 1.08f - edgeNoise * 0.30f) / (sharp ? 9.0f : 12.0f)) * (sharp ? 0.18f : 0.32f);
                var gap = Mathf.Clamp01(Mathf.Sin((normalizedAngle * 1.35f + 48f) * Mathf.Deg2Rad) * 1.8f + 0.70f);
                var alpha = Mathf.Clamp01((mainBand + innerBand + outerBleed) * Mathf.Lerp(0.46f, 1f, broken) * gap);
                tex.SetPixel(x, y, alpha <= 0.015f ? Color.clear : new Color(color.r, color.g, color.b, alpha));
            }
            tex.Apply();
            return Sprite.Create(tex, new Rect(0, 0, size, size), new Vector2(0.5f, 0.5f), 100f);
        }
        ...
        struct ActiveStoppedField
        {
            ...
            public ActiveStoppedField(Vector3 center, float radius, float remaining, float freezeSeconds, bool strong)
            {
                Center = center;
                Radius = radius;
                Remaining = remaining;
    ...
    public sealed class V1ClockHandSweep : MonoBehaviour
    {
        Vector3 pivot;
        float centerDistance;
        float startAngle;
        ...
        public void Configure(Vector3 pivot, float centerDistance, float startAngle, float endAngle, float duration)
        {
            this.pivot = new Vector3(pivot.x, pivot.y, -0.05f);
            this.centerDistance = Mathf.Max(0.01f, centerDistance);
            this.startAngle = startAngle;
            this.endAngle = endAngle;
            this.duration = Mathf.Max(0.02f, duration);
        ...
        void Apply(float t)
        {
            ...
            var dir = Quaternion.Euler(0f, 0f, angle) * Vector3.up;
            transform.position = pivot + dir * centerDistance;
    ...
    public sealed class V1FadingSprite : MonoBehaviour
    {
        ...
        {
            Configure(manager, lifetime, 0f);
        }

        public void Configure(V1GameManager manager, float lifetime, float holdRatio)
        {
            this.manager = manager;
        ...
        void Update()
        {
            ...
            if (sr != null)
            {
                ...
                if (holdRatio > 0f)
                {
                    var fadeStart = lifetime * holdRatio;
                    var fadeDuration = Mathf.Max(0.02f, lifetime - fadeStart);
                    c.a = age <= fadeStart ? startAlpha : Mathf.Lerp(startAlpha, 0f, (age - fadeStart) / fadeDuration);
                }
                else
                {
