...
- Balance status: v0.12 second-pass tuning is in code but not accepted. Growth pace and DPS concentration are near target; first-cycle `망각 전조` survival now reaches the target band in the latest 5-run prelude sample. The next target is stable first-boss TTK measurement with at least 3 gameplay samples past 180s.
