...
- Latest verification status: `node --check src/game.js` passes and repeated browser balance loops execute. Five surfaces were split and tested: prelude survival 10 runs, first-boss TTK 5 runs, twin-blades/hungry-blades 5 runs, greatsword/execution-flash 5 runs, post-loss fast QA, and full-run clear-rate 5 runs. The overall verdict remains `ITERATE_BALANCE`: first-boss prelude survival and weapon/start-memory parity block later balance surfaces.
