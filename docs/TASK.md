...
- [x] 시작 화면에서 무기뿐 아니라 시작 기억도 함께 고른다.
- [x] 시작 빌드는 최소 4개 카드로 제공한다: 쌍검+칼무리, 쌍검+혈반, 대검+칼무리, 대검+혈반.
- [x] 어떤 시작 기억을 골라도 레벨업 카드에서 빠르게 코어 조합(`굶주린 칼무리`, `피의 반사`)을 채울 수 있다.
- [ ] 첫 20~30초 안에 첫 레벨업 또는 강한 보상 선택이 온다.
- [ ] 60~90초 안에 활성 기억 2~3개가 화면에서 돌아가고, 최고 레벨 기억이 다음 망각 후보로 보인다.
- [ ] 첫 망각 전에는 선택한 시작 기억을 충분히 써볼 시간이 있다.
- [ ] 망각 결과 화면은 손실과 보상을 동시에 말한다: `사라진 기억`, `남은 잔향`, `다음 행동`.
- [ ] 120초 압축 리뷰에서는 디버그 자동 완성 없이도 `기억 강화 -> 망각 예고 -> 잔향 기대`가 보인다.

Done criteria:

- [ ] jaewoo가 시작 빌드 카드만 보고 “이번 판 방향”을 이해한다.
...
1. J단계 시작 빌드 선택 구현: 무기+기억 조합 카드.
2. J단계 레벨업 보상 보정: 칼무리/혈반 코어 조합을 빠르게 채우기.
3. C/I단계 120초 압축 리뷰: XP, 카드, 망각 후보, 잔향 기대가 끊기지 않는지 확인.
