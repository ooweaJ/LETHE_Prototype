...
If the date page is displayed inside Project Orchestrator or another host route, do not rely on plain `href="units/..."` links. They can be resolved as `/api/projects/<id>/units/...` and fail. Prefer either:

- same-page hash drill-down, where each card opens an embedded `<article id="unit-YYYY-MM-DD-NN">` detail section, or
- host-aware absolute paths such as `/api/projects/<id>/reports/YYYYMMDD/units/file.html`.

The recommended default is same-page hash drill-down for the human date page, while still generating `units/*.html` for Discord attachments and direct file inspection.
...
- Recommended npm scripts in Node projects:
