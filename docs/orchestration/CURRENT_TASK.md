...
Investigate the latest v0.12 balance-loop failure and prepare the smallest safe balance adjustment.

## Why Now

The latest `npm run balance:loop` returned `ITERATE_BALANCE`: full clear `20%`, death `60%`, first boss clear `100%`, first boss TTK median `26.42s`. Human testing should pause until the balance gate is restored or the user explicitly accepts the risk.
...
- Status, devlog, reports, and decision log are updated with the result.
