...
- `LETHE_UNITY_SLICE_ASSET_PRODUCTION_PLAN.md`: 어떤 이미지를 어떤 순서로 만들고 `_dev`에서 어떻게 검증할지 정하는 제작/테스트 플랜.
...
실제 제작 순서와 `_dev` 테스트 게이트는 `LETHE_UNITY_SLICE_ASSET_PRODUCTION_PLAN.md`를 우선한다.
