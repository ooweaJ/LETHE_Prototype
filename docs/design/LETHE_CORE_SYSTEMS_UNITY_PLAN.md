...
체감 기준은 `LETHE_FORGETTING_FEEL_SPEC.md`를 따른다. 이 문서의 규칙이 플레이 중 큰 장면으로 보이지 않으면 구현 우선순위는 수치 조정보다 망각 결과 화면, 잔향 이펙트, 공명 후보 노출, 궁극 목표 HUD가 먼저다.
